...
import com.epmserver.gateway.service.*;
import com.epmserver.gateway.service.dto.*;
...
import com.epmserver.gateway.service.util.CommonUtil;
...
public class BatchUploadServiceImpl implements BatchUploadService {
    ...
    public BatchUploadServiceImpl(UploadActionTypesRepository uploadActionTypesRepository, UploadTransactionsRepository uploadTransactionsRepository, ProductCategoryExtendRepository productCategoryExtendRepository, UploadTransactionsMapper uploadTransactionsMapper, BarcodeTypesQueryService barcodeTypesQueryService, ProductAttributeQueryService productAttributeQueryService, ProductOptionQueryService productOptionQueryService, MaterialsQueryService materialsQueryService, ProductBrandQueryService productBrandQueryService, WarrantyTypesQueryService warrantyTypesQueryService, ProductsRepository productsRepository, ProductBrandRepository productBrandRepository, BarcodeTypesRepository barcodeTypesRepository, MaterialsRepository materialsRepository, ProductAttributeRepository productAttributeRepository, ProductOptionRepository productOptionRepository, SuppliersExtendService suppliersExtendService, SuppliersMapper suppliersMapper, StockItemTempExtendRepository stockItemTempExtendRepository) {
...
//            uploadTransactions.setTemplateFileContentType(file.getContentType());
            ...
            uploadTransactions.setStatus(UploadTransactionStatus.PENDING);
...
//            savedUploadTransactions = uploadTransactionsRepository.save(savedUploadTransactions);
    ...
    public void importToSystem(Long transactionId, Principal principal) {
        try {
            List<Object[]> productsList = stockItemTempExtendRepository.getProductsFromTemp(transactionId);
            Optional<SuppliersDTO> suppliersDTOOptional = suppliersExtendService.getSupplierByPrincipal(principal);
            Suppliers suppliers = new Suppliers();
            if (suppliersDTOOptional.isPresent()) {
                suppliers = suppliersMapper.toEntity(suppliersDTOOptional.get());
            ...
            for (Object[] productObj : productsList) {
                ...
                List<StockItemTemp> stockItemTempList = stockItemTempExtendRepository.getStockItemTemp(productObj[0].toString());
            ...
            stockItemTempExtendRepository.clearStockItemTemps(transactionId);
