...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.StockItemHoldings}.
 */
@RestController
@RequestMapping("/api")
public class StockItemHoldingsResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/stock-item-holdings?query=:query} : search for the stockItemHoldings corresponding
     * to the query.
     *
     * @param query the query of the stockItemHoldings search.
     * @return the result of the search.
     */
    @GetMapping("/_search/stock-item-holdings")
    public List<StockItemHoldingsDTO> searchStockItemHoldings(@RequestParam String query) {
        log.debug("REST request to search StockItemHoldings for query {}", query);
        return stockItemHoldingsService.search(query);
    }
}
