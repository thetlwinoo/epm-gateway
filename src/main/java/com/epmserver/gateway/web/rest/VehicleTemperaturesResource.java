...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.VehicleTemperatures}.
 */
@RestController
@RequestMapping("/api")
public class VehicleTemperaturesResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/vehicle-temperatures?query=:query} : search for the vehicleTemperatures corresponding
     * to the query.
     *
     * @param query the query of the vehicleTemperatures search.
     * @return the result of the search.
     */
    @GetMapping("/_search/vehicle-temperatures")
    public List<VehicleTemperaturesDTO> searchVehicleTemperatures(@RequestParam String query) {
        log.debug("REST request to search VehicleTemperatures for query {}", query);
        return vehicleTemperaturesService.search(query);
    }
}
