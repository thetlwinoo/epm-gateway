...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.Wishlists}.
 */
@RestController
@RequestMapping("/api")
public class WishlistsResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/wishlists?query=:query} : search for the wishlists corresponding
     * to the query.
     *
     * @param query the query of the wishlists search.
     * @return the result of the search.
     */
    @GetMapping("/_search/wishlists")
    public List<WishlistsDTO> searchWishlists(@RequestParam String query) {
        log.debug("REST request to search Wishlists for query {}", query);
        return wishlistsService.search(query);
    }
}
