...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.TransactionTypes}.
 */
@RestController
@RequestMapping("/api")
public class TransactionTypesResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/transaction-types?query=:query} : search for the transactionTypes corresponding
     * to the query.
     *
     * @param query the query of the transactionTypes search.
     * @return the result of the search.
     */
    @GetMapping("/_search/transaction-types")
    public List<TransactionTypesDTO> searchTransactionTypes(@RequestParam String query) {
        log.debug("REST request to search TransactionTypes for query {}", query);
        return transactionTypesService.search(query);
    }
}
