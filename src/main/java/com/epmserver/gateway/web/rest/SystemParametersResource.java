...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.SystemParameters}.
 */
@RestController
@RequestMapping("/api")
public class SystemParametersResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/system-parameters?query=:query} : search for the systemParameters corresponding
     * to the query.
     *
     * @param query the query of the systemParameters search.
     * @return the result of the search.
     */
    @GetMapping("/_search/system-parameters")
    public List<SystemParametersDTO> searchSystemParameters(@RequestParam String query) {
        log.debug("REST request to search SystemParameters for query {}", query);
        return systemParametersService.search(query);
    }
}
