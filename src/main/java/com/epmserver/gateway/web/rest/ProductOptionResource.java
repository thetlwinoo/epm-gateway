...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.ProductOption}.
 */
@RestController
@RequestMapping("/api")
public class ProductOptionResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/product-options?query=:query} : search for the productOption corresponding
     * to the query.
     *
     * @param query the query of the productOption search.
     * @return the result of the search.
     */
    @GetMapping("/_search/product-options")
    public List<ProductOptionDTO> searchProductOptions(@RequestParam String query) {
        log.debug("REST request to search ProductOptions for query {}", query);
        return productOptionService.search(query);
    }
}
