...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.PackageTypes}.
 */
@RestController
@RequestMapping("/api")
public class PackageTypesResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/package-types?query=:query} : search for the packageTypes corresponding
     * to the query.
     *
     * @param query the query of the packageTypes search.
     * @return the result of the search.
     */
    @GetMapping("/_search/package-types")
    public List<PackageTypesDTO> searchPackageTypes(@RequestParam String query) {
        log.debug("REST request to search PackageTypes for query {}", query);
        return packageTypesService.search(query);
    }
}
