...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.CustomerCategories}.
 */
@RestController
@RequestMapping("/api")
public class CustomerCategoriesResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/customer-categories?query=:query} : search for the customerCategories corresponding
     * to the query.
     *
     * @param query the query of the customerCategories search.
     * @return the result of the search.
     */
    @GetMapping("/_search/customer-categories")
    public List<CustomerCategoriesDTO> searchCustomerCategories(@RequestParam String query) {
        log.debug("REST request to search CustomerCategories for query {}", query);
        return customerCategoriesService.search(query);
    }
}
