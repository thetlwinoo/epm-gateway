...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.StateProvinces}.
 */
@RestController
@RequestMapping("/api")
public class StateProvincesResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/state-provinces?query=:query} : search for the stateProvinces corresponding
     * to the query.
     *
     * @param query the query of the stateProvinces search.
     * @return the result of the search.
     */
    @GetMapping("/_search/state-provinces")
    public List<StateProvincesDTO> searchStateProvinces(@RequestParam String query) {
        log.debug("REST request to search StateProvinces for query {}", query);
        return stateProvincesService.search(query);
    }
}
