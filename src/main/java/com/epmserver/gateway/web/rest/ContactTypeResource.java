...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.ContactType}.
 */
@RestController
@RequestMapping("/api")
public class ContactTypeResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/contact-types?query=:query} : search for the contactType corresponding
     * to the query.
     *
    ...
    public List<ContactTypeDTO> searchContactTypes(@RequestParam String query) {
        log.debug("REST request to search ContactTypes for query {}", query);
        return contactTypeService.search(query);
    }
}
