...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.WishlistProducts}.
 */
@RestController
@RequestMapping("/api")
public class WishlistProductsResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/wishlist-products?query=:query} : search for the wishlistProducts corresponding
     * to the query.
     *
     * @param query the query of the wishlistProducts search.
     * @return the result of the search.
     */
    @GetMapping("/_search/wishlist-products")
    public List<WishlistProductsDTO> searchWishlistProducts(@RequestParam String query) {
        log.debug("REST request to search WishlistProducts for query {}", query);
        return wishlistProductsService.search(query);
    }
}
