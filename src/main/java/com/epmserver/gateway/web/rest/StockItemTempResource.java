...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.StockItemTemp}.
 */
@RestController
@RequestMapping("/api")
public class StockItemTempResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/stock-item-temps?query=:query} : search for the stockItemTemp corresponding
     * to the query.
     *
     * @param query the query of the stockItemTemp search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/stock-item-temps")
    public ResponseEntity<List<StockItemTempDTO>> searchStockItemTemps(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of StockItemTemps for query {}", query);
        Page<StockItemTempDTO> page = stockItemTempService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
