...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.PurchaseOrderLines}.
 */
@RestController
@RequestMapping("/api")
public class PurchaseOrderLinesResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/purchase-order-lines?query=:query} : search for the purchaseOrderLines corresponding
     * to the query.
     *
     * @param query the query of the purchaseOrderLines search.
     * @return the result of the search.
     */
    @GetMapping("/_search/purchase-order-lines")
    public List<PurchaseOrderLinesDTO> searchPurchaseOrderLines(@RequestParam String query) {
        log.debug("REST request to search PurchaseOrderLines for query {}", query);
        return purchaseOrderLinesService.search(query);
    }
}
