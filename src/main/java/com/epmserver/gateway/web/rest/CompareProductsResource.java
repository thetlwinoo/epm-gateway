...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.CompareProducts}.
 */
@RestController
@RequestMapping("/api")
public class CompareProductsResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/compare-products?query=:query} : search for the compareProducts corresponding
     * to the query.
     *
     * @param query the query of the compareProducts search.
     * @return the result of the search.
     */
    @GetMapping("/_search/compare-products")
    public List<CompareProductsDTO> searchCompareProducts(@RequestParam String query) {
        log.debug("REST request to search CompareProducts for query {}", query);
        return compareProductsService.search(query);
    }
}
