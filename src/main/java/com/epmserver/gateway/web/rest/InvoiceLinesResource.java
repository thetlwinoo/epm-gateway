...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.InvoiceLines}.
 */
@RestController
@RequestMapping("/api")
public class InvoiceLinesResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/invoice-lines?query=:query} : search for the invoiceLines corresponding
     * to the query.
     *
     * @param query the query of the invoiceLines search.
     * @return the result of the search.
     */
    @GetMapping("/_search/invoice-lines")
    public List<InvoiceLinesDTO> searchInvoiceLines(@RequestParam String query) {
        log.debug("REST request to search InvoiceLines for query {}", query);
        return invoiceLinesService.search(query);
    }
}
