...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.UploadActionTypes}.
 */
@RestController
@RequestMapping("/api")
public class UploadActionTypesResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/upload-action-types?query=:query} : search for the uploadActionTypes corresponding
     * to the query.
     *
     * @param query the query of the uploadActionTypes search.
     * @return the result of the search.
     */
    @GetMapping("/_search/upload-action-types")
    public List<UploadActionTypesDTO> searchUploadActionTypes(@RequestParam String query) {
        log.debug("REST request to search UploadActionTypes for query {}", query);
        return uploadActionTypesService.search(query);
    }
}
