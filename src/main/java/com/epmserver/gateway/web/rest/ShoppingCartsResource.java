...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.ShoppingCarts}.
 */
@RestController
@RequestMapping("/api")
public class ShoppingCartsResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/shopping-carts?query=:query} : search for the shoppingCarts corresponding
     * to the query.
     *
     * @param query the query of the shoppingCarts search.
     * @return the result of the search.
     */
    @GetMapping("/_search/shopping-carts")
    public List<ShoppingCartsDTO> searchShoppingCarts(@RequestParam String query) {
        log.debug("REST request to search ShoppingCarts for query {}", query);
        return shoppingCartsService.search(query);
    }
}
