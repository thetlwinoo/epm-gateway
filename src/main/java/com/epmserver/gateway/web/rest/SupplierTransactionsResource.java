...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.SupplierTransactions}.
 */
@RestController
@RequestMapping("/api")
public class SupplierTransactionsResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/supplier-transactions?query=:query} : search for the supplierTransactions corresponding
     * to the query.
     *
     * @param query the query of the supplierTransactions search.
     * @return the result of the search.
     */
    @GetMapping("/_search/supplier-transactions")
    public List<SupplierTransactionsDTO> searchSupplierTransactions(@RequestParam String query) {
        log.debug("REST request to search SupplierTransactions for query {}", query);
        return supplierTransactionsService.search(query);
    }
}
