...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.ProductOptionSet}.
 */
@RestController
@RequestMapping("/api")
public class ProductOptionSetResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/product-option-sets?query=:query} : search for the productOptionSet corresponding
     * to the query.
     *
     * @param query the query of the productOptionSet search.
     * @return the result of the search.
     */
    @GetMapping("/_search/product-option-sets")
    public List<ProductOptionSetDTO> searchProductOptionSets(@RequestParam String query) {
        log.debug("REST request to search ProductOptionSets for query {}", query);
        return productOptionSetService.search(query);
    }
}
