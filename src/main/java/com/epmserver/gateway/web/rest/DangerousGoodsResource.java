...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.DangerousGoods}.
 */
@RestController
@RequestMapping("/api")
public class DangerousGoodsResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/dangerous-goods?query=:query} : search for the dangerousGoods corresponding
     * to the query.
     *
     * @param query the query of the dangerousGoods search.
     * @return the result of the search.
     */
    @GetMapping("/_search/dangerous-goods")
    public List<DangerousGoodsDTO> searchDangerousGoods(@RequestParam String query) {
        log.debug("REST request to search DangerousGoods for query {}", query);
        return dangerousGoodsService.search(query);
    }
}
