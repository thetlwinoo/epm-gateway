...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.ProductAttributeSet}.
 */
@RestController
@RequestMapping("/api")
public class ProductAttributeSetResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/product-attribute-sets?query=:query} : search for the productAttributeSet corresponding
     * to the query.
     *
     * @param query the query of the productAttributeSet search.
     * @return the result of the search.
     */
    @GetMapping("/_search/product-attribute-sets")
    public List<ProductAttributeSetDTO> searchProductAttributeSets(@RequestParam String query) {
        log.debug("REST request to search ProductAttributeSets for query {}", query);
        return productAttributeSetService.search(query);
    }
}
