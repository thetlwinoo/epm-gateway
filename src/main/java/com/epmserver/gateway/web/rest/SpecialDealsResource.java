...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.SpecialDeals}.
 */
@RestController
@RequestMapping("/api")
public class SpecialDealsResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/special-deals?query=:query} : search for the specialDeals corresponding
     * to the query.
     *
     * @param query the query of the specialDeals search.
     * @return the result of the search.
     */
    @GetMapping("/_search/special-deals")
    public List<SpecialDealsDTO> searchSpecialDeals(@RequestParam String query) {
        log.debug("REST request to search SpecialDeals for query {}", query);
        return specialDealsService.search(query);
    }
}
