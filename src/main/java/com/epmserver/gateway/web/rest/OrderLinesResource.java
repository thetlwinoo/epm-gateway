...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.OrderLines}.
 */
@RestController
@RequestMapping("/api")
public class OrderLinesResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/order-lines?query=:query} : search for the orderLines corresponding
     * to the query.
     *
     * @param query the query of the orderLines search.
     * @return the result of the search.
     */
    @GetMapping("/_search/order-lines")
    public List<OrderLinesDTO> searchOrderLines(@RequestParam String query) {
        log.debug("REST request to search OrderLines for query {}", query);
        return orderLinesService.search(query);
    }
}
