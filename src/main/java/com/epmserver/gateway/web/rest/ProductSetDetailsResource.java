...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.ProductSetDetails}.
 */
@RestController
@RequestMapping("/api")
public class ProductSetDetailsResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/product-set-details?query=:query} : search for the productSetDetails corresponding
     * to the query.
     *
     * @param query the query of the productSetDetails search.
     * @return the result of the search.
     */
    @GetMapping("/_search/product-set-details")
    public List<ProductSetDetailsDTO> searchProductSetDetails(@RequestParam String query) {
        log.debug("REST request to search ProductSetDetails for query {}", query);
        return productSetDetailsService.search(query);
    }
}
