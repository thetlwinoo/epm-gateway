...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.PhoneNumberType}.
 */
@RestController
@RequestMapping("/api")
public class PhoneNumberTypeResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/phone-number-types?query=:query} : search for the phoneNumberType corresponding
     * to the query.
     *
     * @param query the query of the phoneNumberType search.
     * @return the result of the search.
     */
    @GetMapping("/_search/phone-number-types")
    public List<PhoneNumberTypeDTO> searchPhoneNumberTypes(@RequestParam String query) {
        log.debug("REST request to search PhoneNumberTypes for query {}", query);
        return phoneNumberTypeService.search(query);
    }
}
