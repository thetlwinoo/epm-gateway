...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.WarrantyTypes}.
 */
@RestController
@RequestMapping("/api")
public class WarrantyTypesResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/warranty-types?query=:query} : search for the warrantyTypes corresponding
     * to the query.
     *
     * @param query the query of the warrantyTypes search.
     * @return the result of the search.
     */
    @GetMapping("/_search/warranty-types")
    public List<WarrantyTypesDTO> searchWarrantyTypes(@RequestParam String query) {
        log.debug("REST request to search WarrantyTypes for query {}", query);
        return warrantyTypesService.search(query);
    }
}
