...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.CustomerTransactions}.
 */
@RestController
@RequestMapping("/api")
public class CustomerTransactionsResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/customer-transactions?query=:query} : search for the customerTransactions corresponding
     * to the query.
     *
     * @param query the query of the customerTransactions search.
     * @return the result of the search.
     */
    @GetMapping("/_search/customer-transactions")
    public List<CustomerTransactionsDTO> searchCustomerTransactions(@RequestParam String query) {
        log.debug("REST request to search CustomerTransactions for query {}", query);
        return customerTransactionsService.search(query);
    }
}
