...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.PaymentTransactions}.
 */
@RestController
@RequestMapping("/api")
public class PaymentTransactionsResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/payment-transactions?query=:query} : search for the paymentTransactions corresponding
     * to the query.
     *
     * @param query the query of the paymentTransactions search.
     * @return the result of the search.
     */
    @GetMapping("/_search/payment-transactions")
    public List<PaymentTransactionsDTO> searchPaymentTransactions(@RequestParam String query) {
        log.debug("REST request to search PaymentTransactions for query {}", query);
        return paymentTransactionsService.search(query);
    }
}
