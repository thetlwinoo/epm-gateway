...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.DeliveryMethods}.
 */
@RestController
@RequestMapping("/api")
public class DeliveryMethodsResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/delivery-methods?query=:query} : search for the deliveryMethods corresponding
     * to the query.
     *
     * @param query the query of the deliveryMethods search.
     * @return the result of the search.
     */
    @GetMapping("/_search/delivery-methods")
    public List<DeliveryMethodsDTO> searchDeliveryMethods(@RequestParam String query) {
        log.debug("REST request to search DeliveryMethods for query {}", query);
        return deliveryMethodsService.search(query);
    }
}
