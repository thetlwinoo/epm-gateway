...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.ReviewLines}.
 */
@RestController
@RequestMapping("/api")
public class ReviewLinesResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/review-lines?query=:query} : search for the reviewLines corresponding
     * to the query.
     *
     * @param query the query of the reviewLines search.
     * @return the result of the search.
     */
    @GetMapping("/_search/review-lines")
    public List<ReviewLinesDTO> searchReviewLines(@RequestParam String query) {
        log.debug("REST request to search ReviewLines for query {}", query);
        return reviewLinesService.search(query);
    }
}
