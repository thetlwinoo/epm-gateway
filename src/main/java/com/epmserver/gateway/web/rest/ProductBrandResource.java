...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.ProductBrand}.
 */
@RestController
@RequestMapping("/api")
public class ProductBrandResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/product-brands?query=:query} : search for the productBrand corresponding
     * to the query.
     *
     * @param query the query of the productBrand search.
     * @return the result of the search.
     */
    @GetMapping("/_search/product-brands")
    public List<ProductBrandDTO> searchProductBrands(@RequestParam String query) {
        log.debug("REST request to search ProductBrands for query {}", query);
        return productBrandService.search(query);
    }
}
