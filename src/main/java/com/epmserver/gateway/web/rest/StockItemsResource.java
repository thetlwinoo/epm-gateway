...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.StockItems}.
 */
@RestController
@RequestMapping("/api")
public class StockItemsResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/stock-items?query=:query} : search for the stockItems corresponding
     * to the query.
     *
     * @param query the query of the stockItems search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/stock-items")
    public ResponseEntity<List<StockItemsDTO>> searchStockItems(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of StockItems for query {}", query);
        Page<StockItemsDTO> page = stockItemsService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
