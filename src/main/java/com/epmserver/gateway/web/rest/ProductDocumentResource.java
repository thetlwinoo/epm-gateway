...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.ProductDocument}.
 */
@RestController
@RequestMapping("/api")
public class ProductDocumentResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/product-documents?query=:query} : search for the productDocument corresponding
     * to the query.
     *
    ...
    public List<ProductDocumentDTO> searchProductDocuments(@RequestParam String query) {
        log.debug("REST request to search ProductDocuments for query {}", query);
        return productDocumentService.search(query);
    }
}
