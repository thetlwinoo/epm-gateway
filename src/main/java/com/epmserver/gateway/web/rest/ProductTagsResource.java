...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.ProductTags}.
 */
@RestController
@RequestMapping("/api")
public class ProductTagsResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/product-tags?query=:query} : search for the productTags corresponding
     * to the query.
     *
     * @param query the query of the productTags search.
     * @return the result of the search.
     */
    @GetMapping("/_search/product-tags")
    public List<ProductTagsDTO> searchProductTags(@RequestParam String query) {
        log.debug("REST request to search ProductTags for query {}", query);
        return productTagsService.search(query);
    }
}
