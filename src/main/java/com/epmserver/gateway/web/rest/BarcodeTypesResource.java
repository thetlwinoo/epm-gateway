...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.BarcodeTypes}.
 */
@RestController
@RequestMapping("/api")
public class BarcodeTypesResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/barcode-types?query=:query} : search for the barcodeTypes corresponding
     * to the query.
     *
     * @param query the query of the barcodeTypes search.
     * @return the result of the search.
     */
    @GetMapping("/_search/barcode-types")
    public List<BarcodeTypesDTO> searchBarcodeTypes(@RequestParam String query) {
        log.debug("REST request to search BarcodeTypes for query {}", query);
        return barcodeTypesService.search(query);
    }
}
