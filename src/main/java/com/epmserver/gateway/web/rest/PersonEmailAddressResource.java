...
/**
 * REST controller for managing {@link com.epmserver.gateway.domain.PersonEmailAddress}.
 */
@RestController
@RequestMapping("/api")
public class PersonEmailAddressResource {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/person-email-addresses?query=:query} : search for the personEmailAddress corresponding
     * to the query.
     *
     * @param query the query of the personEmailAddress search.
     * @return the result of the search.
     */
    @GetMapping("/_search/person-email-addresses")
    public List<PersonEmailAddressDTO> searchPersonEmailAddresses(@RequestParam String query) {
        log.debug("REST request to search PersonEmailAddresses for query {}", query);
        return personEmailAddressService.search(query);
    }
}
