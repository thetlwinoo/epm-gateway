...
export class CustomerTransactionsService {
  ...
  public resourceSearchUrl = SERVER_API_URL + 'api/_search/customer-transactions';
  ...
  search(req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http
      .get<ICustomerTransactions[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
      .pipe(map((res: EntityArrayResponseType) => this.convertDateArrayFromServer(res)));
  }
  ...
  protected convertDateFromClient(customerTransactions: ICustomerTransactions): ICustomerTransactions {
    const copy: ICustomerTransactions = Object.assign({}, customerTransactions, {
      transactionOccurWhen:
        customerTransactions.transactionOccurWhen != null && customerTransactions.transactionOccurWhen.isValid()
          ? customerTransactions.transactionOccurWhen.toJSON()
          : null,
      finalizationDate:
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.transactionOccurWhen = res.body.transactionOccurWhen != null ? moment(res.body.transactionOccurWhen) : null;
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((customerTransactions: ICustomerTransactions) => {
        customerTransactions.transactionOccurWhen =
          customerTransactions.transactionOccurWhen != null ? moment(customerTransactions.transactionOccurWhen) : null;
