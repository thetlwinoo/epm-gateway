...
import com.epmserver.gateway.service.dto.WarrantyTypesCriteria;
import com.epmserver.gateway.service.WarrantyTypesQueryService;
...
public class WarrantyTypesResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.WarrantyTypesSearchRepositoryMockConfiguration
     */
    @Autowired
    private WarrantyTypesSearchRepository mockWarrantyTypesSearchRepository;
    ...
    public void createWarrantyTypes() throws Exception {
        ...
        // Validate the WarrantyTypes in Elasticsearch
        verify(mockWarrantyTypesSearchRepository, times(1)).save(testWarrantyTypes);
    }
    ...
    public void createWarrantyTypesWithExistingId() throws Exception {
        ...
        // Validate the WarrantyTypes in Elasticsearch
        verify(mockWarrantyTypesSearchRepository, times(0)).save(warrantyTypes);
    }
    ...
    public void deleteWarrantyTypes() throws Exception {
        ...
        // Validate the WarrantyTypes in Elasticsearch
        verify(mockWarrantyTypesSearchRepository, times(1)).deleteById(warrantyTypes.getId());
    }

    @Test
    @Transactional
    public void searchWarrantyTypes() throws Exception {
        // Initialize the database
        warrantyTypesRepository.saveAndFlush(warrantyTypes);
        when(mockWarrantyTypesSearchRepository.search(queryStringQuery("id:" + warrantyTypes.getId())))
            .thenReturn(Collections.singletonList(warrantyTypes));
        // Search the warrantyTypes
        restWarrantyTypesMockMvc.perform(get("/api/_search/warranty-types?query=id:" + warrantyTypes.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(warrantyTypes.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)));
