...
import com.epmserver.gateway.service.AddressTypesQueryService;
...
public class AddressTypesResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.AddressTypesSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void searchAddressTypes() throws Exception {
        // Initialize the database
        addressTypesRepository.saveAndFlush(addressTypes);
        when(mockAddressTypesSearchRepository.search(queryStringQuery("id:" + addressTypes.getId())))
            .thenReturn(Collections.singletonList(addressTypes));
