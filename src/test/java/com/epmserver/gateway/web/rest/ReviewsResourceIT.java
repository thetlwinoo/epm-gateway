...
import com.epmserver.gateway.service.dto.ReviewsCriteria;
import com.epmserver.gateway.service.ReviewsQueryService;
...
public class ReviewsResourceIT {
    ...
    private static final String DEFAULT_EMAIL_ADDRESS = "n@z.p";
    private static final String UPDATED_EMAIL_ADDRESS = "{@QB.!/";
     ...
     *
     * @see com.epmserver.gateway.repository.search.ReviewsSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createReviews() throws Exception {
        ...
        // Validate the Reviews in Elasticsearch
        verify(mockReviewsSearchRepository, times(1)).save(testReviews);
    }
    ...
    public void createReviewsWithExistingId() throws Exception {
        ...
        // Validate the Reviews in Elasticsearch
        verify(mockReviewsSearchRepository, times(0)).save(reviews);
    }
    ...
    public void deleteReviews() throws Exception {
        ...
        // Validate the Reviews in Elasticsearch
        verify(mockReviewsSearchRepository, times(1)).deleteById(reviews.getId());
    }
    ...
    public void searchReviews() throws Exception {
        // Initialize the database
        reviewsRepository.saveAndFlush(reviews);
        when(mockReviewsSearchRepository.search(queryStringQuery("id:" + reviews.getId())))
            .thenReturn(Collections.singletonList(reviews));
        // Search the reviews
        restReviewsMockMvc.perform(get("/api/_search/reviews?query=id:" + reviews.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(reviews.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)))
