...
import com.epmserver.gateway.service.dto.ShoppingCartsCriteria;
import com.epmserver.gateway.service.ShoppingCartsQueryService;
...
public class ShoppingCartsResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.ShoppingCartsSearchRepositoryMockConfiguration
     */
    @Autowired
    private ShoppingCartsSearchRepository mockShoppingCartsSearchRepository;
    ...
    public void createShoppingCarts() throws Exception {
        ...
        // Validate the ShoppingCarts in Elasticsearch
        verify(mockShoppingCartsSearchRepository, times(1)).save(testShoppingCarts);
    }
    ...
    public void createShoppingCartsWithExistingId() throws Exception {
        ...
        // Validate the ShoppingCarts in Elasticsearch
        verify(mockShoppingCartsSearchRepository, times(0)).save(shoppingCarts);
    }
    ...
    public void deleteShoppingCarts() throws Exception {
        ...
        // Validate the ShoppingCarts in Elasticsearch
        verify(mockShoppingCartsSearchRepository, times(1)).deleteById(shoppingCarts.getId());
    }

    @Test
    @Transactional
    public void searchShoppingCarts() throws Exception {
        // Initialize the database
        shoppingCartsRepository.saveAndFlush(shoppingCarts);
        when(mockShoppingCartsSearchRepository.search(queryStringQuery("id:" + shoppingCarts.getId())))
            .thenReturn(Collections.singletonList(shoppingCarts));
        // Search the shoppingCarts
        restShoppingCartsMockMvc.perform(get("/api/_search/shopping-carts?query=id:" + shoppingCarts.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(shoppingCarts.getId().intValue())))
            .andExpect(jsonPath("$.[*].totalPrice").value(hasItem(DEFAULT_TOTAL_PRICE.intValue())))
