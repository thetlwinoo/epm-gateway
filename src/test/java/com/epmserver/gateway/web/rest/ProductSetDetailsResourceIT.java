...
import com.epmserver.gateway.service.dto.ProductSetDetailsCriteria;
import com.epmserver.gateway.service.ProductSetDetailsQueryService;
...
public class ProductSetDetailsResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.ProductSetDetailsSearchRepositoryMockConfiguration
     */
    @Autowired
    private ProductSetDetailsSearchRepository mockProductSetDetailsSearchRepository;
    ...
    public void createProductSetDetails() throws Exception {
        ...
        // Validate the ProductSetDetails in Elasticsearch
        verify(mockProductSetDetailsSearchRepository, times(1)).save(testProductSetDetails);
    }
    ...
    public void createProductSetDetailsWithExistingId() throws Exception {
        ...
        // Validate the ProductSetDetails in Elasticsearch
        verify(mockProductSetDetailsSearchRepository, times(0)).save(productSetDetails);
    }
    ...
    public void deleteProductSetDetails() throws Exception {
        ...
        // Validate the ProductSetDetails in Elasticsearch
        verify(mockProductSetDetailsSearchRepository, times(1)).deleteById(productSetDetails.getId());
    }

    @Test
    @Transactional
    public void searchProductSetDetails() throws Exception {
        // Initialize the database
        productSetDetailsRepository.saveAndFlush(productSetDetails);
        when(mockProductSetDetailsSearchRepository.search(queryStringQuery("id:" + productSetDetails.getId())))
            .thenReturn(Collections.singletonList(productSetDetails));
        // Search the productSetDetails
        restProductSetDetailsMockMvc.perform(get("/api/_search/product-set-details?query=id:" + productSetDetails.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(productSetDetails.getId().intValue())))
            .andExpect(jsonPath("$.[*].subGroupNo").value(hasItem(DEFAULT_SUB_GROUP_NO)))
