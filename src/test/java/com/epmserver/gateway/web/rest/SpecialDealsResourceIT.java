...
import com.epmserver.gateway.service.dto.SpecialDealsCriteria;
import com.epmserver.gateway.service.SpecialDealsQueryService;
...
public class SpecialDealsResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.SpecialDealsSearchRepositoryMockConfiguration
     */
    @Autowired
    private SpecialDealsSearchRepository mockSpecialDealsSearchRepository;
    ...
    public void createSpecialDeals() throws Exception {
        ...
        // Validate the SpecialDeals in Elasticsearch
        verify(mockSpecialDealsSearchRepository, times(1)).save(testSpecialDeals);
    }
    ...
    public void createSpecialDealsWithExistingId() throws Exception {
        ...
        // Validate the SpecialDeals in Elasticsearch
        verify(mockSpecialDealsSearchRepository, times(0)).save(specialDeals);
    }
    ...
    public void deleteSpecialDeals() throws Exception {
        ...
        // Validate the SpecialDeals in Elasticsearch
        verify(mockSpecialDealsSearchRepository, times(1)).deleteById(specialDeals.getId());
    }

    @Test
    @Transactional
    public void searchSpecialDeals() throws Exception {
        // Initialize the database
        specialDealsRepository.saveAndFlush(specialDeals);
        when(mockSpecialDealsSearchRepository.search(queryStringQuery("id:" + specialDeals.getId())))
            .thenReturn(Collections.singletonList(specialDeals));
        // Search the specialDeals
        restSpecialDealsMockMvc.perform(get("/api/_search/special-deals?query=id:" + specialDeals.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(specialDeals.getId().intValue())))
            .andExpect(jsonPath("$.[*].dealDescription").value(hasItem(DEFAULT_DEAL_DESCRIPTION)))
