...
import com.epmserver.gateway.service.dto.PackageTypesCriteria;
import com.epmserver.gateway.service.PackageTypesQueryService;
...
public class PackageTypesResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.PackageTypesSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createPackageTypes() throws Exception {
        ...
        // Validate the PackageTypes in Elasticsearch
        verify(mockPackageTypesSearchRepository, times(1)).save(testPackageTypes);
    }
    ...
    public void createPackageTypesWithExistingId() throws Exception {
        ...
        // Validate the PackageTypes in Elasticsearch
        verify(mockPackageTypesSearchRepository, times(0)).save(packageTypes);
    }
    ...
    public void deletePackageTypes() throws Exception {
        ...
        // Validate the PackageTypes in Elasticsearch
        verify(mockPackageTypesSearchRepository, times(1)).deleteById(packageTypes.getId());
    }

    @Test
    @Transactional
    public void searchPackageTypes() throws Exception {
        // Initialize the database
        packageTypesRepository.saveAndFlush(packageTypes);
        when(mockPackageTypesSearchRepository.search(queryStringQuery("id:" + packageTypes.getId())))
            .thenReturn(Collections.singletonList(packageTypes));
        // Search the packageTypes
        restPackageTypesMockMvc.perform(get("/api/_search/package-types?query=id:" + packageTypes.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(packageTypes.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)))
