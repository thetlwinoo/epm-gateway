...
import com.epmserver.gateway.service.dto.UploadActionTypesCriteria;
import com.epmserver.gateway.service.UploadActionTypesQueryService;
...
public class UploadActionTypesResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.UploadActionTypesSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createUploadActionTypes() throws Exception {
        ...
        // Validate the UploadActionTypes in Elasticsearch
        verify(mockUploadActionTypesSearchRepository, times(1)).save(testUploadActionTypes);
    }
    ...
    public void createUploadActionTypesWithExistingId() throws Exception {
        ...
        // Validate the UploadActionTypes in Elasticsearch
        verify(mockUploadActionTypesSearchRepository, times(0)).save(uploadActionTypes);
    }
    ...
    public void deleteUploadActionTypes() throws Exception {
        ...
        // Validate the UploadActionTypes in Elasticsearch
        verify(mockUploadActionTypesSearchRepository, times(1)).deleteById(uploadActionTypes.getId());
    }

    @Test
    @Transactional
    public void searchUploadActionTypes() throws Exception {
        // Initialize the database
        uploadActionTypesRepository.saveAndFlush(uploadActionTypes);
        when(mockUploadActionTypesSearchRepository.search(queryStringQuery("id:" + uploadActionTypes.getId())))
            .thenReturn(Collections.singletonList(uploadActionTypes));
        // Search the uploadActionTypes
        restUploadActionTypesMockMvc.perform(get("/api/_search/upload-action-types?query=id:" + uploadActionTypes.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(uploadActionTypes.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)));
