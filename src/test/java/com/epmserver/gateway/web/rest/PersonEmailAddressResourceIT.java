...
import com.epmserver.gateway.service.dto.PersonEmailAddressCriteria;
import com.epmserver.gateway.service.PersonEmailAddressQueryService;
...
public class PersonEmailAddressResourceIT {
    ...
    private static final String DEFAULT_EMAIL_ADDRESS = "Q@*.U";
    private static final String UPDATED_EMAIL_ADDRESS = "G6@l[.t!";
     ...
     *
     * @see com.epmserver.gateway.repository.search.PersonEmailAddressSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createPersonEmailAddress() throws Exception {
        ...
        // Validate the PersonEmailAddress in Elasticsearch
        verify(mockPersonEmailAddressSearchRepository, times(1)).save(testPersonEmailAddress);
    }
    ...
    public void createPersonEmailAddressWithExistingId() throws Exception {
        ...
        // Validate the PersonEmailAddress in Elasticsearch
        verify(mockPersonEmailAddressSearchRepository, times(0)).save(personEmailAddress);
    }
    ...
    public void deletePersonEmailAddress() throws Exception {
        ...
        // Validate the PersonEmailAddress in Elasticsearch
        verify(mockPersonEmailAddressSearchRepository, times(1)).deleteById(personEmailAddress.getId());
    }

    @Test
    @Transactional
    public void searchPersonEmailAddress() throws Exception {
        // Initialize the database
        personEmailAddressRepository.saveAndFlush(personEmailAddress);
        when(mockPersonEmailAddressSearchRepository.search(queryStringQuery("id:" + personEmailAddress.getId())))
            .thenReturn(Collections.singletonList(personEmailAddress));
        // Search the personEmailAddress
        restPersonEmailAddressMockMvc.perform(get("/api/_search/person-email-addresses?query=id:" + personEmailAddress.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(personEmailAddress.getId().intValue())))
            .andExpect(jsonPath("$.[*].emailAddress").value(hasItem(DEFAULT_EMAIL_ADDRESS)))
