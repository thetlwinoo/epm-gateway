...
import com.epmserver.gateway.service.dto.ProductSetCriteria;
import com.epmserver.gateway.service.ProductSetQueryService;
...
public class ProductSetResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.ProductSetSearchRepositoryMockConfiguration
     */
    @Autowired
    private ProductSetSearchRepository mockProductSetSearchRepository;
    ...
    public void createProductSet() throws Exception {
        ...
        // Validate the ProductSet in Elasticsearch
        verify(mockProductSetSearchRepository, times(1)).save(testProductSet);
    }
    ...
    public void createProductSetWithExistingId() throws Exception {
        ...
        // Validate the ProductSet in Elasticsearch
        verify(mockProductSetSearchRepository, times(0)).save(productSet);
    }
    ...
    public void deleteProductSet() throws Exception {
        ...
        // Validate the ProductSet in Elasticsearch
        verify(mockProductSetSearchRepository, times(1)).deleteById(productSet.getId());
    }

    @Test
    @Transactional
    public void searchProductSet() throws Exception {
        // Initialize the database
        productSetRepository.saveAndFlush(productSet);
        when(mockProductSetSearchRepository.search(queryStringQuery("id:" + productSet.getId())))
            .thenReturn(Collections.singletonList(productSet));
        // Search the productSet
        restProductSetMockMvc.perform(get("/api/_search/product-sets?query=id:" + productSet.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(productSet.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)))
