...
public class ProductChoiceResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.ProductChoiceSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createProductChoice() throws Exception {
        ...
        // Validate the ProductChoice in Elasticsearch
        verify(mockProductChoiceSearchRepository, times(1)).save(testProductChoice);
    }
    ...
    public void createProductChoiceWithExistingId() throws Exception {
        ...
        // Validate the ProductChoice in Elasticsearch
        verify(mockProductChoiceSearchRepository, times(0)).save(productChoice);
    }
    ...
    public void deleteProductChoice() throws Exception {
        ...
        // Validate the ProductChoice in Elasticsearch
        verify(mockProductChoiceSearchRepository, times(1)).deleteById(productChoice.getId());
    }

    @Test
    @Transactional
    public void searchProductChoice() throws Exception {
        // Initialize the database
        productChoiceRepository.saveAndFlush(productChoice);
        when(mockProductChoiceSearchRepository.search(queryStringQuery("id:" + productChoice.getId())))
            .thenReturn(Collections.singletonList(productChoice));
        // Search the productChoice
        restProductChoiceMockMvc.perform(get("/api/_search/product-choices?query=id:" + productChoice.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(productChoice.getId().intValue())))
            .andExpect(jsonPath("$.[*].isMultiply").value(hasItem(DEFAULT_IS_MULTIPLY.booleanValue())));
