...
import com.epmserver.gateway.service.dto.StockItemTempCriteria;
import com.epmserver.gateway.service.StockItemTempQueryService;
...
public class StockItemTempResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.StockItemTempSearchRepositoryMockConfiguration
     */
    @Autowired
    private StockItemTempSearchRepository mockStockItemTempSearchRepository;
    ...
    public void createStockItemTemp() throws Exception {
        ...
        // Validate the StockItemTemp in Elasticsearch
        verify(mockStockItemTempSearchRepository, times(1)).save(testStockItemTemp);
    }
    ...
    public void createStockItemTempWithExistingId() throws Exception {
        ...
        // Validate the StockItemTemp in Elasticsearch
        verify(mockStockItemTempSearchRepository, times(0)).save(stockItemTemp);
    }
    ...
    public void deleteStockItemTemp() throws Exception {
        ...
        // Validate the StockItemTemp in Elasticsearch
        verify(mockStockItemTempSearchRepository, times(1)).deleteById(stockItemTemp.getId());
    }

    @Test
    @Transactional
    public void searchStockItemTemp() throws Exception {
        // Initialize the database
        stockItemTempRepository.saveAndFlush(stockItemTemp);
        when(mockStockItemTempSearchRepository.search(queryStringQuery("id:" + stockItemTemp.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(stockItemTemp), PageRequest.of(0, 1), 1));
        // Search the stockItemTemp
        restStockItemTempMockMvc.perform(get("/api/_search/stock-item-temps?query=id:" + stockItemTemp.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(stockItemTemp.getId().intValue())))
            .andExpect(jsonPath("$.[*].stockItemName").value(hasItem(DEFAULT_STOCK_ITEM_NAME)))
