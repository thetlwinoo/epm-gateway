...
import com.epmserver.gateway.service.dto.WishlistsCriteria;
import com.epmserver.gateway.service.WishlistsQueryService;
...
public class WishlistsResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.WishlistsSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createWishlists() throws Exception {
        ...
        // Validate the Wishlists in Elasticsearch
        verify(mockWishlistsSearchRepository, times(1)).save(testWishlists);
    }
    ...
    public void createWishlistsWithExistingId() throws Exception {
        ...
        // Validate the Wishlists in Elasticsearch
        verify(mockWishlistsSearchRepository, times(0)).save(wishlists);
    }
    ...
    public void deleteWishlists() throws Exception {
        ...
        // Validate the Wishlists in Elasticsearch
        verify(mockWishlistsSearchRepository, times(1)).deleteById(wishlists.getId());
    }

    @Test
    @Transactional
    public void searchWishlists() throws Exception {
        // Initialize the database
        wishlistsRepository.saveAndFlush(wishlists);
        when(mockWishlistsSearchRepository.search(queryStringQuery("id:" + wishlists.getId())))
            .thenReturn(Collections.singletonList(wishlists));
        // Search the wishlists
        restWishlistsMockMvc.perform(get("/api/_search/wishlists?query=id:" + wishlists.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(wishlists.getId().intValue())));
    }
