...
import com.epmserver.gateway.service.dto.VehicleTemperaturesCriteria;
import com.epmserver.gateway.service.VehicleTemperaturesQueryService;
...
public class VehicleTemperaturesResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.VehicleTemperaturesSearchRepositoryMockConfiguration
     */
    @Autowired
    private VehicleTemperaturesSearchRepository mockVehicleTemperaturesSearchRepository;
    ...
    public void createVehicleTemperatures() throws Exception {
        ...
        // Validate the VehicleTemperatures in Elasticsearch
        verify(mockVehicleTemperaturesSearchRepository, times(1)).save(testVehicleTemperatures);
    }
    ...
    public void createVehicleTemperaturesWithExistingId() throws Exception {
        ...
        // Validate the VehicleTemperatures in Elasticsearch
        verify(mockVehicleTemperaturesSearchRepository, times(0)).save(vehicleTemperatures);
    }
    ...
    public void deleteVehicleTemperatures() throws Exception {
        ...
        // Validate the VehicleTemperatures in Elasticsearch
        verify(mockVehicleTemperaturesSearchRepository, times(1)).deleteById(vehicleTemperatures.getId());
    }

    @Test
    @Transactional
    public void searchVehicleTemperatures() throws Exception {
        // Initialize the database
        vehicleTemperaturesRepository.saveAndFlush(vehicleTemperatures);
        when(mockVehicleTemperaturesSearchRepository.search(queryStringQuery("id:" + vehicleTemperatures.getId())))
            .thenReturn(Collections.singletonList(vehicleTemperatures));
        // Search the vehicleTemperatures
        restVehicleTemperaturesMockMvc.perform(get("/api/_search/vehicle-temperatures?query=id:" + vehicleTemperatures.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(vehicleTemperatures.getId().intValue())))
            .andExpect(jsonPath("$.[*].vehicleRegistration").value(hasItem(DEFAULT_VEHICLE_REGISTRATION)))
