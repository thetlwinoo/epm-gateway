...
import com.epmserver.gateway.service.dto.UnitMeasureCriteria;
import com.epmserver.gateway.service.UnitMeasureQueryService;
...
public class UnitMeasureResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.UnitMeasureSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createUnitMeasure() throws Exception {
        ...
        // Validate the UnitMeasure in Elasticsearch
        verify(mockUnitMeasureSearchRepository, times(1)).save(testUnitMeasure);
    }
    ...
    public void createUnitMeasureWithExistingId() throws Exception {
        ...
        // Validate the UnitMeasure in Elasticsearch
        verify(mockUnitMeasureSearchRepository, times(0)).save(unitMeasure);
    }
    ...
    public void deleteUnitMeasure() throws Exception {
        ...
        // Validate the UnitMeasure in Elasticsearch
        verify(mockUnitMeasureSearchRepository, times(1)).deleteById(unitMeasure.getId());
    }

    @Test
    @Transactional
    public void searchUnitMeasure() throws Exception {
        // Initialize the database
        unitMeasureRepository.saveAndFlush(unitMeasure);
        when(mockUnitMeasureSearchRepository.search(queryStringQuery("id:" + unitMeasure.getId())))
            .thenReturn(Collections.singletonList(unitMeasure));
        // Search the unitMeasure
        restUnitMeasureMockMvc.perform(get("/api/_search/unit-measures?query=id:" + unitMeasure.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(unitMeasure.getId().intValue())))
            .andExpect(jsonPath("$.[*].code").value(hasItem(DEFAULT_CODE)))
