...
import com.epmserver.gateway.service.BuyingGroupsQueryService;
...
public class BuyingGroupsResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.BuyingGroupsSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createBuyingGroups() throws Exception {
        ...
        // Validate the BuyingGroups in Elasticsearch
        verify(mockBuyingGroupsSearchRepository, times(1)).save(testBuyingGroups);
    }
    ...
    public void createBuyingGroupsWithExistingId() throws Exception {
        ...
        // Validate the BuyingGroups in Elasticsearch
        verify(mockBuyingGroupsSearchRepository, times(0)).save(buyingGroups);
    }
    ...
    public void deleteBuyingGroups() throws Exception {
        ...
        // Validate the BuyingGroups in Elasticsearch
        verify(mockBuyingGroupsSearchRepository, times(1)).deleteById(buyingGroups.getId());
    }

    @Test
    @Transactional
    public void searchBuyingGroups() throws Exception {
        // Initialize the database
        buyingGroupsRepository.saveAndFlush(buyingGroups);
        when(mockBuyingGroupsSearchRepository.search(queryStringQuery("id:" + buyingGroups.getId())))
            .thenReturn(Collections.singletonList(buyingGroups));
        // Search the buyingGroups
        restBuyingGroupsMockMvc.perform(get("/api/_search/buying-groups?query=id:" + buyingGroups.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(buyingGroups.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)))
