...
import com.epmserver.gateway.service.ReviewLinesQueryService;
...
public class ReviewLinesResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.ReviewLinesSearchRepositoryMockConfiguration
     */
    @Autowired
    private ReviewLinesSearchRepository mockReviewLinesSearchRepository;
    ...
    public void createReviewLines() throws Exception {
        ...
        // Validate the ReviewLines in Elasticsearch
        verify(mockReviewLinesSearchRepository, times(1)).save(testReviewLines);
    }
    ...
    public void createReviewLinesWithExistingId() throws Exception {
        ...
        // Validate the ReviewLines in Elasticsearch
        verify(mockReviewLinesSearchRepository, times(0)).save(reviewLines);
    }
    ...
    public void deleteReviewLines() throws Exception {
        ...
        // Validate the ReviewLines in Elasticsearch
        verify(mockReviewLinesSearchRepository, times(1)).deleteById(reviewLines.getId());
    }

    @Test
    @Transactional
    public void searchReviewLines() throws Exception {
        // Initialize the database
        reviewLinesRepository.saveAndFlush(reviewLines);
        when(mockReviewLinesSearchRepository.search(queryStringQuery("id:" + reviewLines.getId())))
            .thenReturn(Collections.singletonList(reviewLines));
        // Search the reviewLines
        restReviewLinesMockMvc.perform(get("/api/_search/review-lines?query=id:" + reviewLines.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(reviewLines.getId().intValue())))
            .andExpect(jsonPath("$.[*].productRating").value(hasItem(DEFAULT_PRODUCT_RATING)))
