...
public class StockItemsResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.StockItemsSearchRepositoryMockConfiguration
     */
    @Autowired
    private StockItemsSearchRepository mockStockItemsSearchRepository;
    ...
    public void createStockItems() throws Exception {
        ...
        // Validate the StockItems in Elasticsearch
        verify(mockStockItemsSearchRepository, times(1)).save(testStockItems);
    }
    ...
    public void createStockItemsWithExistingId() throws Exception {
        ...
        // Validate the StockItems in Elasticsearch
        verify(mockStockItemsSearchRepository, times(0)).save(stockItems);
    }
    ...
    public void deleteStockItems() throws Exception {
        ...
        // Validate the StockItems in Elasticsearch
        verify(mockStockItemsSearchRepository, times(1)).deleteById(stockItems.getId());
    }

    @Test
    @Transactional
    public void searchStockItems() throws Exception {
        // Initialize the database
        stockItemsRepository.saveAndFlush(stockItems);
        when(mockStockItemsSearchRepository.search(queryStringQuery("id:" + stockItems.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(stockItems), PageRequest.of(0, 1), 1));
        // Search the stockItems
        restStockItemsMockMvc.perform(get("/api/_search/stock-items?query=id:" + stockItems.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(stockItems.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)))
