...
import com.epmserver.gateway.service.dto.WishlistProductsCriteria;
import com.epmserver.gateway.service.WishlistProductsQueryService;
...
public class WishlistProductsResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.WishlistProductsSearchRepositoryMockConfiguration
     */
    @Autowired
    private WishlistProductsSearchRepository mockWishlistProductsSearchRepository;
    ...
    public void createWishlistProducts() throws Exception {
        ...
        // Validate the WishlistProducts in Elasticsearch
        verify(mockWishlistProductsSearchRepository, times(1)).save(testWishlistProducts);
    }
    ...
    public void createWishlistProductsWithExistingId() throws Exception {
        ...
        // Validate the WishlistProducts in Elasticsearch
        verify(mockWishlistProductsSearchRepository, times(0)).save(wishlistProducts);
    }
    ...
    public void deleteWishlistProducts() throws Exception {
        ...
        // Validate the WishlistProducts in Elasticsearch
        verify(mockWishlistProductsSearchRepository, times(1)).deleteById(wishlistProducts.getId());
    }

    @Test
    @Transactional
    public void searchWishlistProducts() throws Exception {
        // Initialize the database
        wishlistProductsRepository.saveAndFlush(wishlistProducts);
        when(mockWishlistProductsSearchRepository.search(queryStringQuery("id:" + wishlistProducts.getId())))
            .thenReturn(Collections.singletonList(wishlistProducts));
        // Search the wishlistProducts
        restWishlistProductsMockMvc.perform(get("/api/_search/wishlist-products?query=id:" + wishlistProducts.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(wishlistProducts.getId().intValue())));
    }
