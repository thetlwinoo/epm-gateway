...
import com.epmserver.gateway.service.dto.TransactionTypesCriteria;
import com.epmserver.gateway.service.TransactionTypesQueryService;
...
public class TransactionTypesResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.TransactionTypesSearchRepositoryMockConfiguration
     */
    @Autowired
    private TransactionTypesSearchRepository mockTransactionTypesSearchRepository;
    ...
    public void createTransactionTypes() throws Exception {
        ...
        // Validate the TransactionTypes in Elasticsearch
        verify(mockTransactionTypesSearchRepository, times(1)).save(testTransactionTypes);
    }
    ...
    public void createTransactionTypesWithExistingId() throws Exception {
        ...
        // Validate the TransactionTypes in Elasticsearch
        verify(mockTransactionTypesSearchRepository, times(0)).save(transactionTypes);
    }
    ...
    public void deleteTransactionTypes() throws Exception {
        ...
        // Validate the TransactionTypes in Elasticsearch
        verify(mockTransactionTypesSearchRepository, times(1)).deleteById(transactionTypes.getId());
    }

    @Test
    @Transactional
    public void searchTransactionTypes() throws Exception {
        // Initialize the database
        transactionTypesRepository.saveAndFlush(transactionTypes);
        when(mockTransactionTypesSearchRepository.search(queryStringQuery("id:" + transactionTypes.getId())))
            .thenReturn(Collections.singletonList(transactionTypes));
        // Search the transactionTypes
        restTransactionTypesMockMvc.perform(get("/api/_search/transaction-types?query=id:" + transactionTypes.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(transactionTypes.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)))
