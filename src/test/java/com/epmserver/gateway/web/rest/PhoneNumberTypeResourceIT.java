...
import com.epmserver.gateway.service.dto.PhoneNumberTypeCriteria;
import com.epmserver.gateway.service.PhoneNumberTypeQueryService;
...
public class PhoneNumberTypeResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.PhoneNumberTypeSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createPhoneNumberType() throws Exception {
        ...
        // Validate the PhoneNumberType in Elasticsearch
        verify(mockPhoneNumberTypeSearchRepository, times(1)).save(testPhoneNumberType);
    }
    ...
    public void createPhoneNumberTypeWithExistingId() throws Exception {
        ...
        // Validate the PhoneNumberType in Elasticsearch
        verify(mockPhoneNumberTypeSearchRepository, times(0)).save(phoneNumberType);
    }
    ...
    public void deletePhoneNumberType() throws Exception {
        ...
        // Validate the PhoneNumberType in Elasticsearch
        verify(mockPhoneNumberTypeSearchRepository, times(1)).deleteById(phoneNumberType.getId());
    }

    @Test
    @Transactional
    public void searchPhoneNumberType() throws Exception {
        // Initialize the database
        phoneNumberTypeRepository.saveAndFlush(phoneNumberType);
        when(mockPhoneNumberTypeSearchRepository.search(queryStringQuery("id:" + phoneNumberType.getId())))
            .thenReturn(Collections.singletonList(phoneNumberType));
        // Search the phoneNumberType
        restPhoneNumberTypeMockMvc.perform(get("/api/_search/phone-number-types?query=id:" + phoneNumberType.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(phoneNumberType.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)));
