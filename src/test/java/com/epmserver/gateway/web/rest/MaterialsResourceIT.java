...
import com.epmserver.gateway.service.dto.MaterialsCriteria;
import com.epmserver.gateway.service.MaterialsQueryService;
...
public class MaterialsResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.MaterialsSearchRepositoryMockConfiguration
     */
    @Autowired
    private MaterialsSearchRepository mockMaterialsSearchRepository;
    ...
    public void createMaterials() throws Exception {
        ...
        // Validate the Materials in Elasticsearch
        verify(mockMaterialsSearchRepository, times(1)).save(testMaterials);
    }
    ...
    public void createMaterialsWithExistingId() throws Exception {
        ...
        // Validate the Materials in Elasticsearch
        verify(mockMaterialsSearchRepository, times(0)).save(materials);
    }
    ...
    public void deleteMaterials() throws Exception {
        ...
        // Validate the Materials in Elasticsearch
        verify(mockMaterialsSearchRepository, times(1)).deleteById(materials.getId());
    }

    @Test
    @Transactional
    public void searchMaterials() throws Exception {
        // Initialize the database
        materialsRepository.saveAndFlush(materials);
        when(mockMaterialsSearchRepository.search(queryStringQuery("id:" + materials.getId())))
            .thenReturn(Collections.singletonList(materials));
        // Search the materials
        restMaterialsMockMvc.perform(get("/api/_search/materials?query=id:" + materials.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(materials.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)));
