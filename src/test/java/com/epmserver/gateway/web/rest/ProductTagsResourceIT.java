...
import com.epmserver.gateway.service.dto.ProductTagsCriteria;
import com.epmserver.gateway.service.ProductTagsQueryService;
...
public class ProductTagsResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.ProductTagsSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createProductTags() throws Exception {
        ...
        // Validate the ProductTags in Elasticsearch
        verify(mockProductTagsSearchRepository, times(1)).save(testProductTags);
    }
    ...
    public void createProductTagsWithExistingId() throws Exception {
        ...
        // Validate the ProductTags in Elasticsearch
        verify(mockProductTagsSearchRepository, times(0)).save(productTags);
    }
    ...
    public void deleteProductTags() throws Exception {
        ...
        // Validate the ProductTags in Elasticsearch
        verify(mockProductTagsSearchRepository, times(1)).deleteById(productTags.getId());
    }

    @Test
    @Transactional
    public void searchProductTags() throws Exception {
        // Initialize the database
        productTagsRepository.saveAndFlush(productTags);
        when(mockProductTagsSearchRepository.search(queryStringQuery("id:" + productTags.getId())))
            .thenReturn(Collections.singletonList(productTags));
        // Search the productTags
        restProductTagsMockMvc.perform(get("/api/_search/product-tags?query=id:" + productTags.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(productTags.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)));
