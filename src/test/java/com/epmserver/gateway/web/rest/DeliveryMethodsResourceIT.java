...
import com.epmserver.gateway.service.dto.DeliveryMethodsCriteria;
import com.epmserver.gateway.service.DeliveryMethodsQueryService;
...
public class DeliveryMethodsResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.DeliveryMethodsSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createDeliveryMethods() throws Exception {
        ...
        // Validate the DeliveryMethods in Elasticsearch
        verify(mockDeliveryMethodsSearchRepository, times(1)).save(testDeliveryMethods);
    }
    ...
    public void createDeliveryMethodsWithExistingId() throws Exception {
        ...
        // Validate the DeliveryMethods in Elasticsearch
        verify(mockDeliveryMethodsSearchRepository, times(0)).save(deliveryMethods);
    }
    ...
    public void deleteDeliveryMethods() throws Exception {
        ...
        // Validate the DeliveryMethods in Elasticsearch
        verify(mockDeliveryMethodsSearchRepository, times(1)).deleteById(deliveryMethods.getId());
    }

    @Test
    @Transactional
    public void searchDeliveryMethods() throws Exception {
        // Initialize the database
        deliveryMethodsRepository.saveAndFlush(deliveryMethods);
        when(mockDeliveryMethodsSearchRepository.search(queryStringQuery("id:" + deliveryMethods.getId())))
            .thenReturn(Collections.singletonList(deliveryMethods));
        // Search the deliveryMethods
        restDeliveryMethodsMockMvc.perform(get("/api/_search/delivery-methods?query=id:" + deliveryMethods.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(deliveryMethods.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)))
