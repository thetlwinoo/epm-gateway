...
import com.epmserver.gateway.service.dto.ComparesCriteria;
import com.epmserver.gateway.service.ComparesQueryService;
...
public class ComparesResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.ComparesSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createCompares() throws Exception {
        ...
        // Validate the Compares in Elasticsearch
        verify(mockComparesSearchRepository, times(1)).save(testCompares);
    }
    ...
    public void createComparesWithExistingId() throws Exception {
        ...
        // Validate the Compares in Elasticsearch
        verify(mockComparesSearchRepository, times(0)).save(compares);
    }
    ...
    public void deleteCompares() throws Exception {
        ...
        // Validate the Compares in Elasticsearch
        verify(mockComparesSearchRepository, times(1)).deleteById(compares.getId());
    }

    @Test
    @Transactional
    public void searchCompares() throws Exception {
        // Initialize the database
        comparesRepository.saveAndFlush(compares);
        when(mockComparesSearchRepository.search(queryStringQuery("id:" + compares.getId())))
            .thenReturn(Collections.singletonList(compares));
        // Search the compares
        restComparesMockMvc.perform(get("/api/_search/compares?query=id:" + compares.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(compares.getId().intValue())));
    }
