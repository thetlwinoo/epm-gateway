...
import com.epmserver.gateway.service.dto.ProductCatalogCriteria;
import com.epmserver.gateway.service.ProductCatalogQueryService;
...
public class ProductCatalogResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.ProductCatalogSearchRepositoryMockConfiguration
     */
    @Autowired
    private ProductCatalogSearchRepository mockProductCatalogSearchRepository;
    ...
    public void createProductCatalog() throws Exception {
        ...
        // Validate the ProductCatalog in Elasticsearch
        verify(mockProductCatalogSearchRepository, times(1)).save(testProductCatalog);
    }
    ...
    public void createProductCatalogWithExistingId() throws Exception {
        ...
        // Validate the ProductCatalog in Elasticsearch
        verify(mockProductCatalogSearchRepository, times(0)).save(productCatalog);
    }
    ...
    public void deleteProductCatalog() throws Exception {
        ...
        // Validate the ProductCatalog in Elasticsearch
        verify(mockProductCatalogSearchRepository, times(1)).deleteById(productCatalog.getId());
    }

    @Test
    @Transactional
    public void searchProductCatalog() throws Exception {
        // Initialize the database
        productCatalogRepository.saveAndFlush(productCatalog);
        when(mockProductCatalogSearchRepository.search(queryStringQuery("id:" + productCatalog.getId())))
            .thenReturn(Collections.singletonList(productCatalog));
        // Search the productCatalog
        restProductCatalogMockMvc.perform(get("/api/_search/product-catalogs?query=id:" + productCatalog.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(productCatalog.getId().intValue())));
    }
