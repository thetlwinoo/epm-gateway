...
import com.epmserver.gateway.service.ShipMethodQueryService;
...
public class ShipMethodResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.ShipMethodSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createShipMethod() throws Exception {
        ...
        // Validate the ShipMethod in Elasticsearch
        verify(mockShipMethodSearchRepository, times(1)).save(testShipMethod);
    }
    ...
    public void createShipMethodWithExistingId() throws Exception {
        ...
        // Validate the ShipMethod in Elasticsearch
        verify(mockShipMethodSearchRepository, times(0)).save(shipMethod);
    }
    ...
    public void deleteShipMethod() throws Exception {
        ...
        // Validate the ShipMethod in Elasticsearch
        verify(mockShipMethodSearchRepository, times(1)).deleteById(shipMethod.getId());
    }

    @Test
    @Transactional
    public void searchShipMethod() throws Exception {
        // Initialize the database
        shipMethodRepository.saveAndFlush(shipMethod);
        when(mockShipMethodSearchRepository.search(queryStringQuery("id:" + shipMethod.getId())))
            .thenReturn(Collections.singletonList(shipMethod));
        // Search the shipMethod
        restShipMethodMockMvc.perform(get("/api/_search/ship-methods?query=id:" + shipMethod.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(shipMethod.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)));
