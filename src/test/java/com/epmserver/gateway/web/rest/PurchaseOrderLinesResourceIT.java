...
import com.epmserver.gateway.service.dto.PurchaseOrderLinesCriteria;
import com.epmserver.gateway.service.PurchaseOrderLinesQueryService;
...
public class PurchaseOrderLinesResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.PurchaseOrderLinesSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createPurchaseOrderLines() throws Exception {
        ...
        // Validate the PurchaseOrderLines in Elasticsearch
        verify(mockPurchaseOrderLinesSearchRepository, times(1)).save(testPurchaseOrderLines);
    }
    ...
    public void createPurchaseOrderLinesWithExistingId() throws Exception {
        ...
        // Validate the PurchaseOrderLines in Elasticsearch
        verify(mockPurchaseOrderLinesSearchRepository, times(0)).save(purchaseOrderLines);
    }
    ...
    public void deletePurchaseOrderLines() throws Exception {
        ...
        // Validate the PurchaseOrderLines in Elasticsearch
        verify(mockPurchaseOrderLinesSearchRepository, times(1)).deleteById(purchaseOrderLines.getId());
    }

    @Test
    @Transactional
    public void searchPurchaseOrderLines() throws Exception {
        // Initialize the database
        purchaseOrderLinesRepository.saveAndFlush(purchaseOrderLines);
        when(mockPurchaseOrderLinesSearchRepository.search(queryStringQuery("id:" + purchaseOrderLines.getId())))
            .thenReturn(Collections.singletonList(purchaseOrderLines));
        // Search the purchaseOrderLines
        restPurchaseOrderLinesMockMvc.perform(get("/api/_search/purchase-order-lines?query=id:" + purchaseOrderLines.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(purchaseOrderLines.getId().intValue())))
            .andExpect(jsonPath("$.[*].orderedOuters").value(hasItem(DEFAULT_ORDERED_OUTERS)))
