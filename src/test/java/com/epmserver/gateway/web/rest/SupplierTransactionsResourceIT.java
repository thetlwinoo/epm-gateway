...
import com.epmserver.gateway.service.SupplierTransactionsQueryService;
...
public class SupplierTransactionsResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.SupplierTransactionsSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createSupplierTransactions() throws Exception {
        ...
        // Validate the SupplierTransactions in Elasticsearch
        verify(mockSupplierTransactionsSearchRepository, times(1)).save(testSupplierTransactions);
    }
    ...
    public void createSupplierTransactionsWithExistingId() throws Exception {
        ...
        // Validate the SupplierTransactions in Elasticsearch
        verify(mockSupplierTransactionsSearchRepository, times(0)).save(supplierTransactions);
    }
    ...
    public void deleteSupplierTransactions() throws Exception {
        ...
        // Validate the SupplierTransactions in Elasticsearch
        verify(mockSupplierTransactionsSearchRepository, times(1)).deleteById(supplierTransactions.getId());
    }

    @Test
    @Transactional
    public void searchSupplierTransactions() throws Exception {
        // Initialize the database
        supplierTransactionsRepository.saveAndFlush(supplierTransactions);
        when(mockSupplierTransactionsSearchRepository.search(queryStringQuery("id:" + supplierTransactions.getId())))
            .thenReturn(Collections.singletonList(supplierTransactions));
        // Search the supplierTransactions
        restSupplierTransactionsMockMvc.perform(get("/api/_search/supplier-transactions?query=id:" + supplierTransactions.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(supplierTransactions.getId().intValue())))
            .andExpect(jsonPath("$.[*].supplierInvoiceNumber").value(hasItem(DEFAULT_SUPPLIER_INVOICE_NUMBER)))
