...
import com.epmserver.gateway.service.dto.ShoppingCartItemsCriteria;
import com.epmserver.gateway.service.ShoppingCartItemsQueryService;
...
public class ShoppingCartItemsResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.ShoppingCartItemsSearchRepositoryMockConfiguration
     */
    @Autowired
    private ShoppingCartItemsSearchRepository mockShoppingCartItemsSearchRepository;
    ...
    public void createShoppingCartItems() throws Exception {
        ...
        // Validate the ShoppingCartItems in Elasticsearch
        verify(mockShoppingCartItemsSearchRepository, times(1)).save(testShoppingCartItems);
    }
    ...
    public void createShoppingCartItemsWithExistingId() throws Exception {
        ...
        // Validate the ShoppingCartItems in Elasticsearch
        verify(mockShoppingCartItemsSearchRepository, times(0)).save(shoppingCartItems);
    }
    ...
    public void deleteShoppingCartItems() throws Exception {
        ...
        // Validate the ShoppingCartItems in Elasticsearch
        verify(mockShoppingCartItemsSearchRepository, times(1)).deleteById(shoppingCartItems.getId());
    }

    @Test
    @Transactional
    public void searchShoppingCartItems() throws Exception {
        // Initialize the database
        shoppingCartItemsRepository.saveAndFlush(shoppingCartItems);
        when(mockShoppingCartItemsSearchRepository.search(queryStringQuery("id:" + shoppingCartItems.getId())))
            .thenReturn(Collections.singletonList(shoppingCartItems));
        // Search the shoppingCartItems
        restShoppingCartItemsMockMvc.perform(get("/api/_search/shopping-cart-items?query=id:" + shoppingCartItems.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(shoppingCartItems.getId().intValue())))
            .andExpect(jsonPath("$.[*].quantity").value(hasItem(DEFAULT_QUANTITY)))
