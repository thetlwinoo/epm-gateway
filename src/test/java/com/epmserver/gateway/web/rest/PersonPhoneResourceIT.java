...
import com.epmserver.gateway.service.dto.PersonPhoneCriteria;
import com.epmserver.gateway.service.PersonPhoneQueryService;
...
public class PersonPhoneResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.PersonPhoneSearchRepositoryMockConfiguration
     */
    @Autowired
    private PersonPhoneSearchRepository mockPersonPhoneSearchRepository;
    ...
    public void createPersonPhone() throws Exception {
        ...
        // Validate the PersonPhone in Elasticsearch
        verify(mockPersonPhoneSearchRepository, times(1)).save(testPersonPhone);
    }
    ...
    public void createPersonPhoneWithExistingId() throws Exception {
        ...
        // Validate the PersonPhone in Elasticsearch
        verify(mockPersonPhoneSearchRepository, times(0)).save(personPhone);
    }
    ...
    public void deletePersonPhone() throws Exception {
        ...
        // Validate the PersonPhone in Elasticsearch
        verify(mockPersonPhoneSearchRepository, times(1)).deleteById(personPhone.getId());
    }

    @Test
    @Transactional
    public void searchPersonPhone() throws Exception {
        // Initialize the database
        personPhoneRepository.saveAndFlush(personPhone);
        when(mockPersonPhoneSearchRepository.search(queryStringQuery("id:" + personPhone.getId())))
            .thenReturn(Collections.singletonList(personPhone));
        // Search the personPhone
        restPersonPhoneMockMvc.perform(get("/api/_search/person-phones?query=id:" + personPhone.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(personPhone.getId().intValue())))
            .andExpect(jsonPath("$.[*].phoneNumber").value(hasItem(DEFAULT_PHONE_NUMBER)))
