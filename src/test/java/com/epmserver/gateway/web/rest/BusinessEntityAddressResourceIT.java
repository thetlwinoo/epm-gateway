...
import com.epmserver.gateway.service.dto.BusinessEntityAddressCriteria;
import com.epmserver.gateway.service.BusinessEntityAddressQueryService;
...
public class BusinessEntityAddressResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.BusinessEntityAddressSearchRepositoryMockConfiguration
     */
    @Autowired
    private BusinessEntityAddressSearchRepository mockBusinessEntityAddressSearchRepository;
    ...
    public void createBusinessEntityAddress() throws Exception {
        ...
        // Validate the BusinessEntityAddress in Elasticsearch
        verify(mockBusinessEntityAddressSearchRepository, times(1)).save(testBusinessEntityAddress);
    }
    ...
    public void createBusinessEntityAddressWithExistingId() throws Exception {
        ...
        // Validate the BusinessEntityAddress in Elasticsearch
        verify(mockBusinessEntityAddressSearchRepository, times(0)).save(businessEntityAddress);
    }
    ...
    public void deleteBusinessEntityAddress() throws Exception {
        ...
        // Validate the BusinessEntityAddress in Elasticsearch
        verify(mockBusinessEntityAddressSearchRepository, times(1)).deleteById(businessEntityAddress.getId());
    }

    @Test
    @Transactional
    public void searchBusinessEntityAddress() throws Exception {
        // Initialize the database
        businessEntityAddressRepository.saveAndFlush(businessEntityAddress);
        when(mockBusinessEntityAddressSearchRepository.search(queryStringQuery("id:" + businessEntityAddress.getId())))
            .thenReturn(Collections.singletonList(businessEntityAddress));
        // Search the businessEntityAddress
        restBusinessEntityAddressMockMvc.perform(get("/api/_search/business-entity-addresses?query=id:" + businessEntityAddress.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(businessEntityAddress.getId().intValue())));
    }
