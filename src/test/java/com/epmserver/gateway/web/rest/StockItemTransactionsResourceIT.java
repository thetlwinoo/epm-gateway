...
import com.epmserver.gateway.service.dto.StockItemTransactionsCriteria;
import com.epmserver.gateway.service.StockItemTransactionsQueryService;
...
public class StockItemTransactionsResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.StockItemTransactionsSearchRepositoryMockConfiguration
     */
    @Autowired
    private StockItemTransactionsSearchRepository mockStockItemTransactionsSearchRepository;
    ...
    public void createStockItemTransactions() throws Exception {
        ...
        // Validate the StockItemTransactions in Elasticsearch
        verify(mockStockItemTransactionsSearchRepository, times(1)).save(testStockItemTransactions);
    }
    ...
    public void createStockItemTransactionsWithExistingId() throws Exception {
        ...
        // Validate the StockItemTransactions in Elasticsearch
        verify(mockStockItemTransactionsSearchRepository, times(0)).save(stockItemTransactions);
    }
    ...
    public void deleteStockItemTransactions() throws Exception {
        ...
        // Validate the StockItemTransactions in Elasticsearch
        verify(mockStockItemTransactionsSearchRepository, times(1)).deleteById(stockItemTransactions.getId());
    }

    @Test
    @Transactional
    public void searchStockItemTransactions() throws Exception {
        // Initialize the database
        stockItemTransactionsRepository.saveAndFlush(stockItemTransactions);
        when(mockStockItemTransactionsSearchRepository.search(queryStringQuery("id:" + stockItemTransactions.getId())))
            .thenReturn(Collections.singletonList(stockItemTransactions));
        // Search the stockItemTransactions
        restStockItemTransactionsMockMvc.perform(get("/api/_search/stock-item-transactions?query=id:" + stockItemTransactions.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(stockItemTransactions.getId().intValue())))
            .andExpect(jsonPath("$.[*].transactionOccuredWhen").value(hasItem(DEFAULT_TRANSACTION_OCCURED_WHEN.toString())))
