...
import com.epmserver.gateway.service.dto.SupplierCategoriesCriteria;
import com.epmserver.gateway.service.SupplierCategoriesQueryService;
...
public class SupplierCategoriesResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.SupplierCategoriesSearchRepositoryMockConfiguration
     */
    @Autowired
    private SupplierCategoriesSearchRepository mockSupplierCategoriesSearchRepository;
    ...
    public void createSupplierCategories() throws Exception {
        ...
        // Validate the SupplierCategories in Elasticsearch
        verify(mockSupplierCategoriesSearchRepository, times(1)).save(testSupplierCategories);
    }
    ...
    public void createSupplierCategoriesWithExistingId() throws Exception {
        ...
        // Validate the SupplierCategories in Elasticsearch
        verify(mockSupplierCategoriesSearchRepository, times(0)).save(supplierCategories);
    }
    ...
    public void deleteSupplierCategories() throws Exception {
        ...
        // Validate the SupplierCategories in Elasticsearch
        verify(mockSupplierCategoriesSearchRepository, times(1)).deleteById(supplierCategories.getId());
    }

    @Test
    @Transactional
    public void searchSupplierCategories() throws Exception {
        // Initialize the database
        supplierCategoriesRepository.saveAndFlush(supplierCategories);
        when(mockSupplierCategoriesSearchRepository.search(queryStringQuery("id:" + supplierCategories.getId())))
            .thenReturn(Collections.singletonList(supplierCategories));
        // Search the supplierCategories
        restSupplierCategoriesMockMvc.perform(get("/api/_search/supplier-categories?query=id:" + supplierCategories.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(supplierCategories.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)))
