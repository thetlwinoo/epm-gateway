...
import com.epmserver.gateway.service.dto.StockItemHoldingsCriteria;
import com.epmserver.gateway.service.StockItemHoldingsQueryService;
...
public class StockItemHoldingsResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.StockItemHoldingsSearchRepositoryMockConfiguration
     */
    @Autowired
    private StockItemHoldingsSearchRepository mockStockItemHoldingsSearchRepository;
    ...
    public void createStockItemHoldings() throws Exception {
        ...
        // Validate the StockItemHoldings in Elasticsearch
        verify(mockStockItemHoldingsSearchRepository, times(1)).save(testStockItemHoldings);
    }
    ...
    public void createStockItemHoldingsWithExistingId() throws Exception {
        ...
        // Validate the StockItemHoldings in Elasticsearch
        verify(mockStockItemHoldingsSearchRepository, times(0)).save(stockItemHoldings);
    }
    ...
    public void deleteStockItemHoldings() throws Exception {
        ...
        // Validate the StockItemHoldings in Elasticsearch
        verify(mockStockItemHoldingsSearchRepository, times(1)).deleteById(stockItemHoldings.getId());
    }

    @Test
    @Transactional
    public void searchStockItemHoldings() throws Exception {
        // Initialize the database
        stockItemHoldingsRepository.saveAndFlush(stockItemHoldings);
        when(mockStockItemHoldingsSearchRepository.search(queryStringQuery("id:" + stockItemHoldings.getId())))
            .thenReturn(Collections.singletonList(stockItemHoldings));
        // Search the stockItemHoldings
        restStockItemHoldingsMockMvc.perform(get("/api/_search/stock-item-holdings?query=id:" + stockItemHoldings.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(stockItemHoldings.getId().intValue())))
            .andExpect(jsonPath("$.[*].quantityOnHand").value(hasItem(DEFAULT_QUANTITY_ON_HAND)))
