...
public class CitiesResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.CitiesSearchRepositoryMockConfiguration
     */
    @Autowired
    private CitiesSearchRepository mockCitiesSearchRepository;
    ...
    public void createCities() throws Exception {
        ...
        // Validate the Cities in Elasticsearch
        verify(mockCitiesSearchRepository, times(1)).save(testCities);
    }
    ...
    public void createCitiesWithExistingId() throws Exception {
        ...
        // Validate the Cities in Elasticsearch
        verify(mockCitiesSearchRepository, times(0)).save(cities);
    }
    ...
    public void deleteCities() throws Exception {
        ...
        // Validate the Cities in Elasticsearch
        verify(mockCitiesSearchRepository, times(1)).deleteById(cities.getId());
    }
    ...
    public void searchCities() throws Exception {
        // Initialize the database
        citiesRepository.saveAndFlush(cities);
        when(mockCitiesSearchRepository.search(queryStringQuery("id:" + cities.getId())))
            .thenReturn(Collections.singletonList(cities));
        // Search the cities
        restCitiesMockMvc.perform(get("/api/_search/cities?query=id:" + cities.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(cities.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)))
