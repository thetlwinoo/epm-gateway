...
import com.epmserver.gateway.service.dto.CustomerTransactionsCriteria;
import com.epmserver.gateway.service.CustomerTransactionsQueryService;
...
public class CustomerTransactionsResourceIT {
    ...
    private static final Instant DEFAULT_TRANSACTION_OCCUR_WHEN = Instant.ofEpochMilli(0L);
    private static final Instant UPDATED_TRANSACTION_OCCUR_WHEN = Instant.now().truncatedTo(ChronoUnit.MILLIS);
     ...
     *
     * @see com.epmserver.gateway.repository.search.CustomerTransactionsSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public static CustomerTransactions createEntity(EntityManager em) {
        CustomerTransactions customerTransactions = new CustomerTransactions()
            .transactionOccurWhen(DEFAULT_TRANSACTION_OCCUR_WHEN)
    ...
    public static CustomerTransactions createUpdatedEntity(EntityManager em) {
        CustomerTransactions customerTransactions = new CustomerTransactions()
            .transactionOccurWhen(UPDATED_TRANSACTION_OCCUR_WHEN)
    ...
    public void createCustomerTransactions() throws Exception {
        ...
        assertThat(testCustomerTransactions.getTransactionOccurWhen()).isEqualTo(DEFAULT_TRANSACTION_OCCUR_WHEN);
        ...
        // Validate the CustomerTransactions in Elasticsearch
        verify(mockCustomerTransactionsSearchRepository, times(1)).save(testCustomerTransactions);
    }
    ...
    public void createCustomerTransactionsWithExistingId() throws Exception {
        ...
        // Validate the CustomerTransactions in Elasticsearch
        verify(mockCustomerTransactionsSearchRepository, times(0)).save(customerTransactions);
    }


    @Test
    @Transactional
    public void checkTransactionOccurWhenIsRequired() throws Exception {
        int databaseSizeBeforeTest = customerTransactionsRepository.findAll().size();
        // set the field null
        customerTransactions.setTransactionOccurWhen(null);
    ...
    public void getAllCustomerTransactions() throws Exception {
        ...
        restCustomerTransactionsMockMvc.perform(get("/api/customer-transactions?sort=id,desc"))
            ...
            .andExpect(jsonPath("$.[*].transactionOccurWhen").value(hasItem(DEFAULT_TRANSACTION_OCCUR_WHEN.toString())))
    ...
    public void getCustomerTransactions() throws Exception {
        ...
        restCustomerTransactionsMockMvc.perform(get("/api/customer-transactions/{id}", customerTransactions.getId()))
            ...
            .andExpect(jsonPath("$.transactionOccurWhen").value(DEFAULT_TRANSACTION_OCCUR_WHEN.toString()))
    ...
    @Test
    @Transactional
    public void getAllCustomerTransactionsByTransactionOccurWhenIsEqualToSomething() throws Exception {
        // Initialize the database
        customerTransactionsRepository.saveAndFlush(customerTransactions);

        // Get all the customerTransactionsList where transactionOccurWhen equals to DEFAULT_TRANSACTION_OCCUR_WHEN
        defaultCustomerTransactionsShouldBeFound("transactionOccurWhen.equals=" + DEFAULT_TRANSACTION_OCCUR_WHEN);

        // Get all the customerTransactionsList where transactionOccurWhen equals to UPDATED_TRANSACTION_OCCUR_WHEN
        defaultCustomerTransactionsShouldNotBeFound("transactionOccurWhen.equals=" + UPDATED_TRANSACTION_OCCUR_WHEN);
    }

    @Test
    @Transactional
    public void getAllCustomerTransactionsByTransactionOccurWhenIsNotEqualToSomething() throws Exception {
        // Initialize the database
        customerTransactionsRepository.saveAndFlush(customerTransactions);

        // Get all the customerTransactionsList where transactionOccurWhen not equals to DEFAULT_TRANSACTION_OCCUR_WHEN
        defaultCustomerTransactionsShouldNotBeFound("transactionOccurWhen.notEquals=" + DEFAULT_TRANSACTION_OCCUR_WHEN);

        // Get all the customerTransactionsList where transactionOccurWhen not equals to UPDATED_TRANSACTION_OCCUR_WHEN
        defaultCustomerTransactionsShouldBeFound("transactionOccurWhen.notEquals=" + UPDATED_TRANSACTION_OCCUR_WHEN);
    }

    @Test
    @Transactional
    public void getAllCustomerTransactionsByTransactionOccurWhenIsInShouldWork() throws Exception {
        // Initialize the database
        customerTransactionsRepository.saveAndFlush(customerTransactions);

        // Get all the customerTransactionsList where transactionOccurWhen in DEFAULT_TRANSACTION_OCCUR_WHEN or UPDATED_TRANSACTION_OCCUR_WHEN
        defaultCustomerTransactionsShouldBeFound("transactionOccurWhen.in=" + DEFAULT_TRANSACTION_OCCUR_WHEN + "," + UPDATED_TRANSACTION_OCCUR_WHEN);

        // Get all the customerTransactionsList where transactionOccurWhen equals to UPDATED_TRANSACTION_OCCUR_WHEN
        defaultCustomerTransactionsShouldNotBeFound("transactionOccurWhen.in=" + UPDATED_TRANSACTION_OCCUR_WHEN);
    }

    @Test
    @Transactional
    public void getAllCustomerTransactionsByTransactionOccurWhenIsNullOrNotNull() throws Exception {
        // Initialize the database
        customerTransactionsRepository.saveAndFlush(customerTransactions);

        // Get all the customerTransactionsList where transactionOccurWhen is not null
        defaultCustomerTransactionsShouldBeFound("transactionOccurWhen.specified=true");

        // Get all the customerTransactionsList where transactionOccurWhen is null
        defaultCustomerTransactionsShouldNotBeFound("transactionOccurWhen.specified=false");
    }
    ...
    public void updateCustomerTransactions() throws Exception {
        ...
        CustomerTransactions testCustomerTransactions = customerTransactionsList.get(customerTransactionsList.size() - 1);
        assertThat(testCustomerTransactions.getTransactionOccurWhen()).isEqualTo(UPDATED_TRANSACTION_OCCUR_WHEN);
    ...
    public void deleteCustomerTransactions() throws Exception {
        ...
        // Validate the CustomerTransactions in Elasticsearch
        verify(mockCustomerTransactionsSearchRepository, times(1)).deleteById(customerTransactions.getId());
    }

    @Test
    @Transactional
    public void searchCustomerTransactions() throws Exception {
        // Initialize the database
        customerTransactionsRepository.saveAndFlush(customerTransactions);
        when(mockCustomerTransactionsSearchRepository.search(queryStringQuery("id:" + customerTransactions.getId())))
            .thenReturn(Collections.singletonList(customerTransactions));
        // Search the customerTransactions
        restCustomerTransactionsMockMvc.perform(get("/api/_search/customer-transactions?query=id:" + customerTransactions.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(customerTransactions.getId().intValue())))
            .andExpect(jsonPath("$.[*].transactionOccurWhen").value(hasItem(DEFAULT_TRANSACTION_OCCUR_WHEN.toString())))
