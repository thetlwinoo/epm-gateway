...
import com.epmserver.gateway.service.dto.ProductOptionSetCriteria;
import com.epmserver.gateway.service.ProductOptionSetQueryService;
...
public class ProductOptionSetResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.ProductOptionSetSearchRepositoryMockConfiguration
     */
    @Autowired
    private ProductOptionSetSearchRepository mockProductOptionSetSearchRepository;
    ...
    public void createProductOptionSet() throws Exception {
        ...
        // Validate the ProductOptionSet in Elasticsearch
        verify(mockProductOptionSetSearchRepository, times(1)).save(testProductOptionSet);
    }
    ...
    public void createProductOptionSetWithExistingId() throws Exception {
        ...
        // Validate the ProductOptionSet in Elasticsearch
        verify(mockProductOptionSetSearchRepository, times(0)).save(productOptionSet);
    }
    ...
    public void deleteProductOptionSet() throws Exception {
        ...
        // Validate the ProductOptionSet in Elasticsearch
        verify(mockProductOptionSetSearchRepository, times(1)).deleteById(productOptionSet.getId());
    }

    @Test
    @Transactional
    public void searchProductOptionSet() throws Exception {
        // Initialize the database
        productOptionSetRepository.saveAndFlush(productOptionSet);
        when(mockProductOptionSetSearchRepository.search(queryStringQuery("id:" + productOptionSet.getId())))
            .thenReturn(Collections.singletonList(productOptionSet));
        // Search the productOptionSet
        restProductOptionSetMockMvc.perform(get("/api/_search/product-option-sets?query=id:" + productOptionSet.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(productOptionSet.getId().intValue())))
            .andExpect(jsonPath("$.[*].value").value(hasItem(DEFAULT_VALUE)));
