...
import com.epmserver.gateway.service.dto.OrderLinesCriteria;
import com.epmserver.gateway.service.OrderLinesQueryService;
...
import com.epmserver.gateway.domain.enumeration.OrderLineStatus;
/**
...
public class OrderLinesResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.OrderLinesSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createOrderLines() throws Exception {
        ...
        // Validate the OrderLines in Elasticsearch
        verify(mockOrderLinesSearchRepository, times(1)).save(testOrderLines);
    }
    ...
    public void createOrderLinesWithExistingId() throws Exception {
        ...
        // Validate the OrderLines in Elasticsearch
        verify(mockOrderLinesSearchRepository, times(0)).save(orderLines);
    }
    ...
    public void deleteOrderLines() throws Exception {
        ...
        // Validate the OrderLines in Elasticsearch
        verify(mockOrderLinesSearchRepository, times(1)).deleteById(orderLines.getId());
    }

    @Test
    @Transactional
    public void searchOrderLines() throws Exception {
        // Initialize the database
        orderLinesRepository.saveAndFlush(orderLines);
        when(mockOrderLinesSearchRepository.search(queryStringQuery("id:" + orderLines.getId())))
            .thenReturn(Collections.singletonList(orderLines));
        // Search the orderLines
        restOrderLinesMockMvc.perform(get("/api/_search/order-lines?query=id:" + orderLines.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(orderLines.getId().intValue())))
            .andExpect(jsonPath("$.[*].carrierTrackingNumber").value(hasItem(DEFAULT_CARRIER_TRACKING_NUMBER)))
