...
import com.epmserver.gateway.service.DangerousGoodsQueryService;
...
public class DangerousGoodsResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.DangerousGoodsSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createDangerousGoods() throws Exception {
        ...
        // Validate the DangerousGoods in Elasticsearch
        verify(mockDangerousGoodsSearchRepository, times(1)).save(testDangerousGoods);
    }
    ...
    public void createDangerousGoodsWithExistingId() throws Exception {
        ...
        // Validate the DangerousGoods in Elasticsearch
        verify(mockDangerousGoodsSearchRepository, times(0)).save(dangerousGoods);
    }
    ...
    public void deleteDangerousGoods() throws Exception {
        ...
        // Validate the DangerousGoods in Elasticsearch
        verify(mockDangerousGoodsSearchRepository, times(1)).deleteById(dangerousGoods.getId());
    }

    @Test
    @Transactional
    public void searchDangerousGoods() throws Exception {
        // Initialize the database
        dangerousGoodsRepository.saveAndFlush(dangerousGoods);
        when(mockDangerousGoodsSearchRepository.search(queryStringQuery("id:" + dangerousGoods.getId())))
            .thenReturn(Collections.singletonList(dangerousGoods));
        // Search the dangerousGoods
        restDangerousGoodsMockMvc.perform(get("/api/_search/dangerous-goods?query=id:" + dangerousGoods.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(dangerousGoods.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)));
