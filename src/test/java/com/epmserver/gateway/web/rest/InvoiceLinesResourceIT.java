...
import com.epmserver.gateway.service.dto.InvoiceLinesCriteria;
import com.epmserver.gateway.service.InvoiceLinesQueryService;
...
public class InvoiceLinesResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.InvoiceLinesSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createInvoiceLines() throws Exception {
        ...
        // Validate the InvoiceLines in Elasticsearch
        verify(mockInvoiceLinesSearchRepository, times(1)).save(testInvoiceLines);
    }
    ...
    public void createInvoiceLinesWithExistingId() throws Exception {
        ...
        // Validate the InvoiceLines in Elasticsearch
        verify(mockInvoiceLinesSearchRepository, times(0)).save(invoiceLines);
    }
    ...
    public void deleteInvoiceLines() throws Exception {
        ...
        // Validate the InvoiceLines in Elasticsearch
        verify(mockInvoiceLinesSearchRepository, times(1)).deleteById(invoiceLines.getId());
    }

    @Test
    @Transactional
    public void searchInvoiceLines() throws Exception {
        // Initialize the database
        invoiceLinesRepository.saveAndFlush(invoiceLines);
        when(mockInvoiceLinesSearchRepository.search(queryStringQuery("id:" + invoiceLines.getId())))
            .thenReturn(Collections.singletonList(invoiceLines));
        // Search the invoiceLines
        restInvoiceLinesMockMvc.perform(get("/api/_search/invoice-lines?query=id:" + invoiceLines.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(invoiceLines.getId().intValue())))
            .andExpect(jsonPath("$.[*].description").value(hasItem(DEFAULT_DESCRIPTION)))
