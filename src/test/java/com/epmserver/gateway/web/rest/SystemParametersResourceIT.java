...
import com.epmserver.gateway.service.dto.SystemParametersCriteria;
import com.epmserver.gateway.service.SystemParametersQueryService;
...
public class SystemParametersResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.SystemParametersSearchRepositoryMockConfiguration
     */
    @Autowired
    private SystemParametersSearchRepository mockSystemParametersSearchRepository;
    ...
    public void createSystemParameters() throws Exception {
        ...
        // Validate the SystemParameters in Elasticsearch
        verify(mockSystemParametersSearchRepository, times(1)).save(testSystemParameters);
    }
    ...
    public void createSystemParametersWithExistingId() throws Exception {
        ...
        // Validate the SystemParameters in Elasticsearch
        verify(mockSystemParametersSearchRepository, times(0)).save(systemParameters);
    }
    ...
    public void deleteSystemParameters() throws Exception {
        ...
        // Validate the SystemParameters in Elasticsearch
        verify(mockSystemParametersSearchRepository, times(1)).deleteById(systemParameters.getId());
    }

    @Test
    @Transactional
    public void searchSystemParameters() throws Exception {
        // Initialize the database
        systemParametersRepository.saveAndFlush(systemParameters);
        when(mockSystemParametersSearchRepository.search(queryStringQuery("id:" + systemParameters.getId())))
            .thenReturn(Collections.singletonList(systemParameters));
        // Search the systemParameters
        restSystemParametersMockMvc.perform(get("/api/_search/system-parameters?query=id:" + systemParameters.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(systemParameters.getId().intValue())))
            .andExpect(jsonPath("$.[*].applicationSettings").value(hasItem(DEFAULT_APPLICATION_SETTINGS)));
