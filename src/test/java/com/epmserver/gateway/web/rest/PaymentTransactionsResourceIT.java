...
import com.epmserver.gateway.service.dto.PaymentTransactionsCriteria;
import com.epmserver.gateway.service.PaymentTransactionsQueryService;
...
public class PaymentTransactionsResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.PaymentTransactionsSearchRepositoryMockConfiguration
     */
    @Autowired
    private PaymentTransactionsSearchRepository mockPaymentTransactionsSearchRepository;
    ...
    public void createPaymentTransactions() throws Exception {
        ...
        // Validate the PaymentTransactions in Elasticsearch
        verify(mockPaymentTransactionsSearchRepository, times(1)).save(testPaymentTransactions);
    }
    ...
    public void createPaymentTransactionsWithExistingId() throws Exception {
        ...
        // Validate the PaymentTransactions in Elasticsearch
        verify(mockPaymentTransactionsSearchRepository, times(0)).save(paymentTransactions);
    }
    ...
    public void deletePaymentTransactions() throws Exception {
        ...
        // Validate the PaymentTransactions in Elasticsearch
        verify(mockPaymentTransactionsSearchRepository, times(1)).deleteById(paymentTransactions.getId());
    }

    @Test
    @Transactional
    public void searchPaymentTransactions() throws Exception {
        // Initialize the database
        paymentTransactionsRepository.saveAndFlush(paymentTransactions);
        when(mockPaymentTransactionsSearchRepository.search(queryStringQuery("id:" + paymentTransactions.getId())))
            .thenReturn(Collections.singletonList(paymentTransactions));
        // Search the paymentTransactions
        restPaymentTransactionsMockMvc.perform(get("/api/_search/payment-transactions?query=id:" + paymentTransactions.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(paymentTransactions.getId().intValue())))
            .andExpect(jsonPath("$.[*].returnedCompletedPaymentData").value(hasItem(DEFAULT_RETURNED_COMPLETED_PAYMENT_DATA.toString())))
