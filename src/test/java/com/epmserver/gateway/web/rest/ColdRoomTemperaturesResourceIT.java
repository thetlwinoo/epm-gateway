...
import com.epmserver.gateway.service.dto.ColdRoomTemperaturesCriteria;
import com.epmserver.gateway.service.ColdRoomTemperaturesQueryService;
...
public class ColdRoomTemperaturesResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.ColdRoomTemperaturesSearchRepositoryMockConfiguration
     */
    @Autowired
    private ColdRoomTemperaturesSearchRepository mockColdRoomTemperaturesSearchRepository;
    ...
    public void createColdRoomTemperatures() throws Exception {
        ...
        // Validate the ColdRoomTemperatures in Elasticsearch
        verify(mockColdRoomTemperaturesSearchRepository, times(1)).save(testColdRoomTemperatures);
    }
    ...
    public void createColdRoomTemperaturesWithExistingId() throws Exception {
        ...
        // Validate the ColdRoomTemperatures in Elasticsearch
        verify(mockColdRoomTemperaturesSearchRepository, times(0)).save(coldRoomTemperatures);
    }
    ...
    public void deleteColdRoomTemperatures() throws Exception {
        ...
        // Validate the ColdRoomTemperatures in Elasticsearch
        verify(mockColdRoomTemperaturesSearchRepository, times(1)).deleteById(coldRoomTemperatures.getId());
    }

    @Test
    @Transactional
    public void searchColdRoomTemperatures() throws Exception {
        // Initialize the database
        coldRoomTemperaturesRepository.saveAndFlush(coldRoomTemperatures);
        when(mockColdRoomTemperaturesSearchRepository.search(queryStringQuery("id:" + coldRoomTemperatures.getId())))
            .thenReturn(Collections.singletonList(coldRoomTemperatures));
        // Search the coldRoomTemperatures
        restColdRoomTemperaturesMockMvc.perform(get("/api/_search/cold-room-temperatures?query=id:" + coldRoomTemperatures.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(coldRoomTemperatures.getId().intValue())))
            .andExpect(jsonPath("$.[*].coldRoomSensorNumber").value(hasItem(DEFAULT_COLD_ROOM_SENSOR_NUMBER)))
