...
import com.epmserver.gateway.service.dto.ProductBrandCriteria;
import com.epmserver.gateway.service.ProductBrandQueryService;
...
public class ProductBrandResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.ProductBrandSearchRepositoryMockConfiguration
     */
    @Autowired
    private ProductBrandSearchRepository mockProductBrandSearchRepository;
    ...
    public void createProductBrand() throws Exception {
        ...
        // Validate the ProductBrand in Elasticsearch
        verify(mockProductBrandSearchRepository, times(1)).save(testProductBrand);
    }
    ...
    public void createProductBrandWithExistingId() throws Exception {
        ...
        // Validate the ProductBrand in Elasticsearch
        verify(mockProductBrandSearchRepository, times(0)).save(productBrand);
    }
    ...
    public void deleteProductBrand() throws Exception {
        ...
        // Validate the ProductBrand in Elasticsearch
        verify(mockProductBrandSearchRepository, times(1)).deleteById(productBrand.getId());
    }

    @Test
    @Transactional
    public void searchProductBrand() throws Exception {
        // Initialize the database
        productBrandRepository.saveAndFlush(productBrand);
        when(mockProductBrandSearchRepository.search(queryStringQuery("id:" + productBrand.getId())))
            .thenReturn(Collections.singletonList(productBrand));
        // Search the productBrand
        restProductBrandMockMvc.perform(get("/api/_search/product-brands?query=id:" + productBrand.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(productBrand.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)))
