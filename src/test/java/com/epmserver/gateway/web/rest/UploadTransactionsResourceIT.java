...
import com.epmserver.gateway.service.dto.UploadTransactionsCriteria;
import com.epmserver.gateway.service.UploadTransactionsQueryService;
...
public class UploadTransactionsResourceIT {
    ...
    private static final UploadTransactionStatus DEFAULT_STATUS = UploadTransactionStatus.COMPLETED;
    private static final UploadTransactionStatus UPDATED_STATUS = UploadTransactionStatus.PENDING;
     ...
     *
     * @see com.epmserver.gateway.repository.search.UploadTransactionsSearchRepositoryMockConfiguration
     */
    @Autowired
    private UploadTransactionsSearchRepository mockUploadTransactionsSearchRepository;
    ...
    public void createUploadTransactions() throws Exception {
        ...
        // Validate the UploadTransactions in Elasticsearch
        verify(mockUploadTransactionsSearchRepository, times(1)).save(testUploadTransactions);
    }
    ...
    public void createUploadTransactionsWithExistingId() throws Exception {
        ...
        // Validate the UploadTransactions in Elasticsearch
        verify(mockUploadTransactionsSearchRepository, times(0)).save(uploadTransactions);
    }
    ...
    public void checkStatusIsRequired() throws Exception {
        ...
        // set the field null
        uploadTransactions.setStatus(null);
    ...
    public void deleteUploadTransactions() throws Exception {
        ...
        // Validate the UploadTransactions in Elasticsearch
        verify(mockUploadTransactionsSearchRepository, times(1)).deleteById(uploadTransactions.getId());
    }

    @Test
    @Transactional
    public void searchUploadTransactions() throws Exception {
        // Initialize the database
        uploadTransactionsRepository.saveAndFlush(uploadTransactions);
        when(mockUploadTransactionsSearchRepository.search(queryStringQuery("id:" + uploadTransactions.getId())))
            .thenReturn(Collections.singletonList(uploadTransactions));
        // Search the uploadTransactions
        restUploadTransactionsMockMvc.perform(get("/api/_search/upload-transactions?query=id:" + uploadTransactions.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(uploadTransactions.getId().intValue())))
            .andExpect(jsonPath("$.[*].fileName").value(hasItem(DEFAULT_FILE_NAME)))
