...
import com.epmserver.gateway.service.dto.CultureCriteria;
import com.epmserver.gateway.service.CultureQueryService;
...
public class CultureResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.CultureSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createCulture() throws Exception {
        ...
        // Validate the Culture in Elasticsearch
        verify(mockCultureSearchRepository, times(1)).save(testCulture);
    }
    ...
    public void createCultureWithExistingId() throws Exception {
        ...
        // Validate the Culture in Elasticsearch
        verify(mockCultureSearchRepository, times(0)).save(culture);
    }
    ...
    public void deleteCulture() throws Exception {
        ...
        // Validate the Culture in Elasticsearch
        verify(mockCultureSearchRepository, times(1)).deleteById(culture.getId());
    }

    @Test
    @Transactional
    public void searchCulture() throws Exception {
        // Initialize the database
        cultureRepository.saveAndFlush(culture);
        when(mockCultureSearchRepository.search(queryStringQuery("id:" + culture.getId())))
            .thenReturn(Collections.singletonList(culture));
        // Search the culture
        restCultureMockMvc.perform(get("/api/_search/cultures?query=id:" + culture.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(culture.getId().intValue())))
            .andExpect(jsonPath("$.[*].code").value(hasItem(DEFAULT_CODE)))
