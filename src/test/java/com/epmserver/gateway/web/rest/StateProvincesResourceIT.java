...
import com.epmserver.gateway.service.dto.StateProvincesCriteria;
import com.epmserver.gateway.service.StateProvincesQueryService;
...
public class StateProvincesResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.StateProvincesSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createStateProvinces() throws Exception {
        ...
        // Validate the StateProvinces in Elasticsearch
        verify(mockStateProvincesSearchRepository, times(1)).save(testStateProvinces);
    }
    ...
    public void createStateProvincesWithExistingId() throws Exception {
        ...
        // Validate the StateProvinces in Elasticsearch
        verify(mockStateProvincesSearchRepository, times(0)).save(stateProvinces);
    }
    ...
    public void deleteStateProvinces() throws Exception {
        ...
        // Validate the StateProvinces in Elasticsearch
        verify(mockStateProvincesSearchRepository, times(1)).deleteById(stateProvinces.getId());
    }

    @Test
    @Transactional
    public void searchStateProvinces() throws Exception {
        // Initialize the database
        stateProvincesRepository.saveAndFlush(stateProvinces);
        when(mockStateProvincesSearchRepository.search(queryStringQuery("id:" + stateProvinces.getId())))
            .thenReturn(Collections.singletonList(stateProvinces));
        // Search the stateProvinces
        restStateProvincesMockMvc.perform(get("/api/_search/state-provinces?query=id:" + stateProvinces.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(stateProvinces.getId().intValue())))
            .andExpect(jsonPath("$.[*].code").value(hasItem(DEFAULT_CODE)))
