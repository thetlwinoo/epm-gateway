...
import com.epmserver.gateway.service.dto.CompareProductsCriteria;
import com.epmserver.gateway.service.CompareProductsQueryService;
...
public class CompareProductsResourceIT {
     ...
     *
     * @see com.epmserver.gateway.repository.search.CompareProductsSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createCompareProducts() throws Exception {
        ...
        // Validate the CompareProducts in Elasticsearch
        verify(mockCompareProductsSearchRepository, times(1)).save(testCompareProducts);
    }
    ...
    public void createCompareProductsWithExistingId() throws Exception {
        ...
        // Validate the CompareProducts in Elasticsearch
        verify(mockCompareProductsSearchRepository, times(0)).save(compareProducts);
    }
    ...
    public void deleteCompareProducts() throws Exception {
        ...
        // Validate the CompareProducts in Elasticsearch
        verify(mockCompareProductsSearchRepository, times(1)).deleteById(compareProducts.getId());
    }

    @Test
    @Transactional
    public void searchCompareProducts() throws Exception {
        // Initialize the database
        compareProductsRepository.saveAndFlush(compareProducts);
        when(mockCompareProductsSearchRepository.search(queryStringQuery("id:" + compareProducts.getId())))
            .thenReturn(Collections.singletonList(compareProducts));
        // Search the compareProducts
        restCompareProductsMockMvc.perform(get("/api/_search/compare-products?query=id:" + compareProducts.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(compareProducts.getId().intValue())));
    }
